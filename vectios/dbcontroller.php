<?php
	...
	class controller
	{
		...
		public function getPerfil($dbcon,$userId,$DEBUG_STATUS)
		{
			...
			u.u_profile_pic,u.u_emailId,u.u_personal_interest,u.u_business_interest from z_users u where u_id=".$userId;
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result)) 
				{
					...
					$usrDtl[8] = $row["u_personal_interest"];
					$usrDtl[9] = $row["u_business_interest"];
				}
			}
			return $usrDtl;
		}

		public function updateProfileIntereses($dbcon,$userInteresesPersonal,$userInteresesNegocio,$DEBUG_STATUS)
		{
			
			$sql = "UPDATE z_users set u_personal_interest='".$userInteresesPersonal."',u_business_interest='".$userInteresesNegocio."'
		...
		public function getDetallesViajesNacional($dbcon,$ciudad_origen,$ciudad_destino,$horavuelo,$sector,$destino,$nroasientes,$mascotas,$fumar,$alcohol,$DEBUG_STATUS)
		{
			...
			$sql="select pn.id, (select a.a_placa from z_automovil a where a.a_id=pn.id_bus) placa ,
					(select CONCAT(a_marca,' ',a_modelo,'-',a_ano)  from z_automovil a where a.a_id=pn.id_bus) auto_dtl,
					pn.fecha_salida, 
					(select c.c_name from z_ciudad c where c.c_id=pn.lugar_salida) lugar_salida,
					(select c.c_name from z_ciudad c where c.c_id=pn.lugar_destino) lugar_destino,
					pn.monto
					from plan_nacional pn 
					where pn.fecha_salida <= DATE_ADD(DATE_FORMAT('".$horavuelo."','%Y-%m-%d %H:%i:%s'),INTERVAL 5 HOUR) and pn.fecha_salida >= DATE_SUB(DATE_FORMAT('".$horavuelo."','%Y-%m-%d %H:%i:%s'),INTERVAL 5 HOUR) and pn.lugar_salida=".$ciudad_origen." and pn.lugar_destino=".$ciudad_destino;
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$viajeDtl[$count] = array($row["id"],$row["placa"],$row["auto_dtl"],$row["fecha_salida"],$row["lugar_salida"],$row["lugar_destino"],$row["monto"]);
					$count++;
				}
			}
		...
		public function getDetallesViajesNacionalById($dbcon,$id,$DEBUG_STATUS)
		{
			$viajeDtl=array();
		...
		public function reservarViajeNacional($dbcon,$idviaje,$target_dir,$tipoPago,$direccion,$cantpass,$userid,$costo,$DEBUG_STATUS)
		{
			...
			$viajeDtl='';
			$codigo_viaje=mt_rand();
			$total_pago=$cantpass*$costo;

			mysqli_autocommit($dbcon,FALSE);
			$target_file = $target_dir .'PagoPic-'.$codigo_viaje.'-'.$userid.'.jpg';
			$sql="insert into z_viajespasajero(vp_viaje_id,vp_fecha_acceptation,vp_fecha_pago, vp_pic_pago,vp_tipo_pago,vp_estado_viaje,vp_comprado_por,vp_sector_inicio,vp_sector_destino,vp_codigo_pago,vp_codigo_pago_ref,vp_valor_total_pago,vp_nro_pasajeros,vp_fecha_salida) 
			 		values(".$idviaje.",now(),now(),'".$target_file."',".$tipoPago.",2,".$userid.",4,5,12345,1214420200614103058,".$total_pago.",".$cantpass.",now())";
			if($DEBUG_STATUS)
            ...
            {
            	mysqli_commit($dbcon);
            	return $idviaje.':'.$ctr.':'.$cantpass.':'.$sql; 
            }
            else
            {
            	mysqli_rollback($dbcon);
            	return '0:'.$ctr.':'.$cantpass.':'.$sql;
            }
		...
		public function getSectorsMain($dbcon,$cuidad,$DEBUG_STATUS)
		{
		...
		public function getTerminalsBySector($dbcon,$cuidad,$sector,$DEBUG_STATUS)
		{
			$sql="select distinct t.t_id,t.t_name from z_terminal t where t.t_id >1 and t.t_sector=".$sector." and t.t_estado=0 
					and t.t_id_ciudad=".$cuidad." order by t.t_name asc";
			$terminal=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
		...
		public function getTerminalDesc($dbcon,$terminal_id,$DEBUG_STATUS)
		{
			$sql="select t.t_name from z_terminal t where t.t_id=".$terminal_id;
			$result = mysqli_query($dbcon,$sql);
			$t_name='';
            if(mysqli_num_rows($result) > 0)  
            {
		...
		public function misreservas($dbcon,$userId,$DEBUG_STATUS)
		{
			...
			/*$sql="select vp_viaje_id,s1.sector_name desde,s2.sector_name hasta,vp.vp_fecha_salida, vp.vp_estado_viaje codigo_estado,
					...
					vp_codigo_pago,
					vp_id,
					vp_pago_verificado,vp_observacion,vp_nro_pasajeros,
					(select nro_retornos_comprado from z_viajescupon vc where vc.id_pago=vp.vp_codigo_pago) nro_retornos_comprado,    
					(select (nro_retornos_comprado-nro_retornos_planificado) from z_viajescupon vc where vc.id_pago=vp.vp_codigo_pago) nro_retornos_pendiente 
					from z_viajespasajero vp, z_sector s1,z_sector s2
					where vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_comprado_por=".$userId." order by vp.vp_fecha_salida desc";*/
				/*$sql="select vp_viaje_id,s1.sector_name desde,s2.sector_name hasta,vp.vp_fecha_salida, vp.vp_estado_viaje codigo_estado,
					(select p_desc from z_parametros where p_id=vp.vp_estado_viaje) vp_estado_viaje,
					(vp.vp_valor_total_pago+(select valor_retorno_pagado from z_viajescupon vc where vp.vp_codigo_pago=vc.id_pago)) vp_valor_total_pago,
					vp.vp_tipo_pago,vp.vp_fecha_acceptation,
					(case when vp.vp_fecha_salida<now() then 1 else 0 end) open_terminate,
					(case when vp.vp_sector_inicio=1 then 1 else 0 end) return_journey,
					vp_codigo_pago,
					...
					(select nro_retornos_comprado from z_viajescupon vc where vc.id_pago=vp.vp_codigo_pago) nro_retornos_comprado,    
					(select (nro_retornos_comprado-nro_retornos_planificado) from z_viajescupon vc where vc.id_pago=vp.vp_codigo_pago) nro_retornos_pendiente,
					vp_viaje_inicio is_viaje_inicio
					from z_viajespasajero vp, z_sector s1,z_sector s2
					where vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_comprado_por=".$userId." order by vp.vp_fecha_salida";*/
				$sql="select vp_viaje_id,s1.sector_name desde,s2.sector_name hasta,
					(select t.t_name from z_terminal t where t.t_id=vp.vp_terminal_inicio) terminal_inicio,
					(select t2.t_name from z_terminal t2 where t2.t_id=vp.vp_terminal_destino) terminal_destino,
					vp.vp_fecha_salida, vp.vp_estado_viaje codigo_estado,
					(select p_desc from z_parametros where p_id=vp.vp_estado_viaje) vp_estado_viaje,
					(vp.vp_valor_total_pago+(select valor_retorno_pagado from z_viajescupon vc where vp.vp_codigo_pago_ref=vc.id_pago_ref)) vp_valor_total_pago,
					vp.vp_tipo_pago,vp.vp_fecha_acceptation,
					(case when vp.vp_fecha_salida<now() then 1 else 0 end) open_terminate,
					(case when vp.vp_sector_inicio=1 then 1 else 0 end) return_journey,
					vp_codigo_pago,vp_codigo_pago_ref,
					vp_id,
					vp_pago_verificado,vp_observacion,vp_nro_pasajeros,
					(select nro_retornos_comprado from z_viajescupon vc where vc.id_pago_ref=vp.vp_codigo_pago_ref) nro_retornos_comprado,    
					(select (nro_retornos_comprado-nro_retornos_planificado) from z_viajescupon vc where vc.id_pago_ref=vp.vp_codigo_pago_ref) nro_retornos_pendiente,
					vp_viaje_inicio is_viaje_inicio,vp.vp_sector_destino
					from z_viajespasajero vp, z_sector s1,z_sector s2
					where vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_comprado_por=".$userId." order by vp.vp_fecha_salida";
			$misreservas=array();
			$count=0;
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					if($row["vp_sector_destino"]==1)
					{
						$info_desde=$row["desde"].' '.$row["terminal_inicio"];
						$info_hasta=$row["hasta"];
					}
					else
					{
						$info_desde=$row["desde"];
						$info_hasta=$row["hasta"].' '.$row["terminal_destino"];
					}
					$misreservas[$count] = array(	$row["vp_viaje_id"],
													...
													$row["nro_retornos_comprado"],
													$row["nro_retornos_pendiente"],
													$row["is_viaje_inicio"],
													$row["vp_codigo_pago_ref"],
													$row["vp_sector_destino"]);
					$count++;
				}
			}
			return $misreservas;
		...
		public function detallesParaCambiarConductor($dbcon,$viajeId,$DEBUG_STATUS)
		{
			$sql="select v.v_viaje_id,s1.sector_name desde,s2.sector_name hasta,vp.vp_fecha_salida,
				(select t.t_name from z_terminal t where t.t_id=vp.vp_terminal_inicio) terminal_inicio,
				(select t2.t_name from z_terminal t2 where t2.t_id=vp.vp_terminal_destino) terminal_destino,
				...
				sum(vp.vp_nro_pasajeros) vp_nro_pasajeros,vp.vp_doc_paymet_to_client,vp.vp_dt_payment_to_client,
				v.v_id_conductor,
				(select u.u_name from z_users u where u.u_id=v.v_id_conductor) conductor_name,
				vp.vp_estado_viaje vp_codigo_estado_viaje, 
				(case when vp.vp_fecha_salida<DATE_SUB(now(),INTERVAL 3 DAY) then 1 else 0 end) do_terminate 
				from z_viajes v, z_viajespasajero vp,z_sector s1,z_sector s2
				where v.v_viaje_id like '%".$viajeId."%' and v.v_viaje_id=vp.vp_viaje_id and s1.id=vp.vp_sector_inicio and s2.id=vp.vp_sector_destino";
			/*if($estado!=99 && $estado!=3)
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					$conductor_data = '['.$row["v_id_conductor"].'] '.$row["conductor_name"];
					$misreservas[$count] = array(	$row["v_viaje_id"],
												$info_desde,
												$info_hasta,
												$row["vp_fecha_salida"],
		...
		public function cambiarConductor($dbcon,$id_conductor,$viaje_id,$DEBUG_STATUS)
		{
			mysqli_autocommit($dbcon,FALSE);
			$sql="update z_viajes set v_id_conductor=".$id_conductor." where v_viaje_id=".$viaje_id;
			//echo 'SQL:'.$sql.'<br>';
		...
		public function controlPagos($dbcon,$DEBUG_STATUS)
		{
			...
			$sql = "select vp.vp_viaje_id,vp.vp_codigo_pago_ref,vp.vp_fecha_pago,vp.vp_tipo_pago,vp.vp_pic_pago,u.u_name,vp.vp_observacion,
						vp.vp_pago_verificado,(vp.vp_valor_total_pago+valor_retorno_pagado) vp_valor_total_pago,vp.vp_fecha_pago_en_banco,vp.vp_codigo_pago
						from z_viajespasajero vp,z_viajescupon vc,z_users u 
						where vp_codigo_pago_ref=id_pago_ref and vp.vp_pago_verificado=99 and 
						vp.vp_comprado_por=u.u_id and vp.vp_codigo_pago_ref not like'%TMP_%' 
						group by vp.vp_viaje_id,vp.vp_codigo_pago_ref,vp.vp_fecha_pago,vp.vp_tipo_pago,vp.vp_pic_pago,u.u_name,vp.vp_observacion,
						vp.vp_pago_verificado,vp.vp_fecha_pago_en_banco order by vp.vp_fecha_pago";
			$docs=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
		...
		public function controlPagosById($dbcon,$pago_id,$DEBUG_STATUS)
		{
			...
			$sql="select vp.vp_codigo_pago_ref,vp.vp_fecha_pago,vp.vp_fecha_pago_en_banco,vp.vp_tipo_pago,vp.vp_pic_pago,u.u_name,vp.vp_observacion,
					vp.vp_pago_verificado,(vp.vp_valor_total_pago+valor_retorno_pagado) vp_valor_total_pago,vp.vp_codigo_pago from z_viajespasajero vp,z_viajescupon vc,z_users u where ";
			if(!empty($pago_id))
				$sql=$sql." vp.vp_codigo_pago_ref like '%".$pago_id."%' and";  
			$sql=$sql." vp.vp_comprado_por=u.u_id and vp_codigo_pago_ref=vc.id_pago_ref group by vp.vp_codigo_pago order by vp.vp_fecha_pago";
			$docs=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					$docs[$count] = array(1,$row["vp_codigo_pago_ref"],$row["vp_fecha_pago"],$strPago,$row["vp_pic_pago"],$row["u_name"],$row["vp_observacion"],$row["vp_pago_verificado"],$row["vp_valor_total_pago"],$row["vp_fecha_pago_en_banco"],$row["vp_codigo_pago"]);
					$count++;
				}
			}
			return $docs;
		...
		public function updatePagoVerification($dbcon,$Id,$estado,$observacion,$DEBUG_STATUS)
		{
			...
			if($estado==1)
				$sql="update z_viajespasajero set vp_pago_verificado=".$estado.",vp_estado_viaje=2,vp_observacion='".$observacion."' where vp_codigo_pago_ref='".$Id."'";
			else
				$sql="update z_viajespasajero set vp_pago_verificado=".$estado.",vp_estado_viaje=7,vp_observacion='".$observacion."' where vp_codigo_pago_ref='".$Id."'";
		...
		public function getTopCommentsForHomePage($dbcon,$DEBUG_STATUS)
		{
			$sql="select c_comments, c_commented_by,c_commented_on,(select u.u_name from z_users u where u.u_id=c.c_commented_by) u_name,
			(select u.u_profile_pic from z_users u where u.u_id=c.c_commented_by) u_profile_pic
				from z_comments c where c.c_enable_for_home_page=1 ";
			$comments=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
		...
		public function terminateViaje($dbcon,$codigo_viaje,$codigo_pago,$userId,$DEBUG_STATUS)
		{
			...
			mysqli_autocommit($dbcon,FALSE);
			$err_code=1;
			$sql = "UPDATE z_viajespasajero SET vp_estado_viaje=3 WHERE vp_viaje_id=$codigo_viaje and vp_codigo_pago_ref='".$codigo_pago."' and vp_comprado_por=$userId";
			//if($DEBUG_STATUS)
				echo '$sql-1::'.$sql.'<br>';
	        if(mysqli_query($dbcon,$sql) && mysqli_affected_rows($dbcon)>0)
	        {
		...
		public function regsiterViajePlanificado($dbcon,$sector,$terminal,$nroasientessearch,$fechaviajesearch,$calle_principal,$numeracion,$calle_secundario,$referencia,$codigoRetorno,$DEBUG_STATUS)
		{
			mysqli_autocommit($dbcon,FALSE);
			$err_code=0;
			$ctr=0;
        	...
        	/*if(isset($cantidadRetorno) && $cantidadRetorno>2)
        		$valorViajeRetorno=15.00;
        	else
        		$valorViajeRetorno=6.00*$cantidadRetorno;*/
        	if(isset($codigoRetorno) && $codigoRetorno==0) 
        	{
        		$cantidadRetorno=0;
        		$valorViajeRetorno=0.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno>0 && $codigoRetorno<=4) 
        	{
        		$cantidadRetorno=$codigoRetorno;
        		$valorViajeRetorno=8.00*$cantidadRetorno;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==5) 
        	{
        		$cantidadRetorno=2;
        		$valorViajeRetorno=14.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==6) 
        	{
        		$cantidadRetorno=4;
        		$valorViajeRetorno=18.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==7) 
        	{
        		$cantidadRetorno=3;
        		$valorViajeRetorno=22.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==8) 
        	{
        		$cantidadRetorno=4;
        		$valorViajeRetorno=28.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==9) 
        	{
        		$cantidadRetorno=4;
        		$valorViajeRetorno=26.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==10) 
        	{
        		$cantidadRetorno=4;
        		$valorViajeRetorno=30.00;
        	}
        	else if(isset($codigoRetorno) && $codigoRetorno==101) 
        	{
        		$cantidadRetorno=1;
        		$valorViajeRetorno=15.00;
        	}
			...
			$sqlPago = "select concat('TMP_','".$codigo_pago_temporal."',date_format(now(),'%Y%m%d%H%i%s')) pago from dual";

			$result = mysqli_query($dbcon,$sqlPago);
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result)) 
				{
					$strPagoId = $row["pago"];
		       		...
		       		$sql = "insert into z_viajespasajero(vp_sector_inicio,vp_terminal_inicio,vp_sector_destino,vp_codigo_pago,vp_codigo_pago_ref,vp_valor_total_pago,vp_nro_pasajeros,vp_comprado_por,vp_codigo_identification,vp_fecha_salida,vp_calle_principal,vp_numeracion,vp_calle_secundaria,vp_referencia,vp_estado_viaje,vp_codigo_confirmacion,vp_fecha_acceptation) values 
							(".$sector.",".$terminal.",1,'TMP_".$codigo_pago_temporal."','".$strPagoId."',".$valorViaje.",".$nroasientessearch.",".$_SESSION['userid'].",(select u_cedula from z_users where u_id='".$_SESSION['userid']."'),DATE_FORMAT('".$fechaviajesearch."','%Y-%m-%d %H:%i:%s'),'".$calle_principal."','".$numeracion."','".$calle_secundario."','".$referencia."',1,'".mt_rand()."',now())";
			        ...
			        if(mysqli_query($dbcon,$sql))
			        {
			        	$sql = "insert into z_viajescupon(id_pago_ref,nro_retornos_comprado,nro_retornos_planificado,valor_retorno_pagado,codigo_retorno) values 
								('".$strPagoId."',".$cantidadRetorno.",0,".$valorViajeRetorno.",".$codigoRetorno.")";
						if($DEBUG_STATUS)
							echo '$sql-2::'.$sql.'<br>';
				        if(mysqli_query($dbcon,$sql))
				        {
				        	...
				        	if(isset($codigoRetorno) && $codigoRetorno==0)
				        	{
				        		$sql = "insert into z_viajesretornoplan(id_pago_ref,nro_asientos) values 
									('".$strPagoId."',0)";									
						        if(mysqli_query($dbcon,$sql))
						        {
						        	mysqli_commit($dbcon);
		...
		public function regsiterViajePlanificadoCiudad($dbcon,$sector,$terminal,$nroasientessearch,$fechaviajesearch,$calle_principal,$numeracion,$calle_secundario,$referencia,$cantidadRetorno,$cedulas,$DEBUG_STATUS)
		{
			mysqli_autocommit($dbcon,FALSE);
			$err_code=0;
			$ctr=0;
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				...
				{
					$strPagoId = $row["pago"];
		       		$sql = "insert into z_viajespasajero(vp_sector_inicio,vp_sector_destino,vp_terminal_destino,vp_codigo_pago,vp_codigo_pago_ref,vp_valor_total_pago,vp_nro_pasajeros,vp_comprado_por,vp_codigo_identification,vp_fecha_salida,vp_calle_principal,vp_numeracion,vp_calle_secundaria,vp_referencia,vp_estado_viaje,vp_codigo_confirmacion,vp_fecha_acceptation) values 
							(1,".$sector.",".$terminal.",'TMP_".$codigo_pago_temporal."','".$strPagoId."',".$valorViaje.",".$nroasientessearch.",".$_SESSION['userid'].",(select u_cedula from z_users where u_id='".$_SESSION['userid']."'),DATE_FORMAT('".$fechaviajesearch."','%Y-%m-%d %H:%i:%s'),'".$calle_principal."','".$numeracion."','".$calle_secundario."','".$referencia."',1,'".mt_rand()."',now())";
					if($DEBUG_STATUS)
						echo '$sql-1::'.$sql.'<br>';
			        if(mysqli_query($dbcon,$sql))
			        {
			        	...
			        	{
			        		$sql = "insert into z_viajesPasajerosRetorno(id_pago,nro_identificacion,estado) values 
								('TMP_".$codigo_pago_temporal."','".$cedulas[$ctr]."',0)";
							if($DEBUG_STATUS)
								echo '$sql-2::'.$sql.'<br>';
					        if(mysqli_query($dbcon,$sql))
					        {
			        	...
			        	{*/
				        	$sql = "insert into z_viajescupon(id_pago_ref,nro_retornos_comprado,nro_retornos_planificado,valor_retorno_pagado) values 
								('".$strPagoId."',".$cantidadRetorno.",0,".$valorViajeRetorno.")";
							if($DEBUG_STATUS)
								echo '$sql-2::'.$sql.'<br>';
					        if(mysqli_query($dbcon,$sql))
					        {
		...
		public function regsiterViajePlanificadoRetornoCiudad($dbcon,$sector,$terminal,$nroasientessearch,$fechaviajesearch,$calle_principal,$numeracion,$calle_secundario,$referencia,$cedulas,$c_viaje,$nroasientesid,$DEBUG_STATUS)
		{
			mysqli_autocommit($dbcon,FALSE);
			$err_code=0;
			$ctr=0;
	        ...
	        {
				$cedulaStr=$cedulaStr.$cedulas[$ctr].',';
				$ctr++;
			}
			
			$sql="select vp_codigo_pago,vp_codigo_pago_ref,vc.nro_retornos_comprado,vc.valor_retorno_pagado,vc.codigo_retorno from z_viajespasajero vp,z_viajescupon vc where vp.vp_id=".$c_viaje." and vp.vp_codigo_pago_ref=vc.id_pago_ref";
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result)) 
				{	
					if($row["codigo_retorno"]==101)
					{
						if($nroasientessearch==1)
							$valor=15;
					}
					...
					$sql = "insert into z_viajespasajero(vp_sector_inicio,vp_sector_destino,vp_terminal_destino,vp_codigo_pago,vp_codigo_pago_ref,vp_valor_total_pago,vp_nro_pasajeros,vp_comprado_por,vp_codigo_identification,vp_fecha_salida,vp_calle_principal,vp_numeracion,vp_calle_secundaria,vp_referencia,vp_estado_viaje,vp_codigo_confirmacion,vp_fecha_acceptation,vp_pago_verificado,vp_viaje_inicio) values 
								(1,".$sector.",".$terminal.",'".$row["vp_codigo_pago"]."','".$row["vp_codigo_pago_ref"]."',".$valor.",".$nroasientessearch.",".$_SESSION['userid'].",'".$cedulaStr."',DATE_FORMAT('".$fechaviajesearch."','%Y-%m-%d %H:%i:%s'),'".$calle_principal."','".$numeracion."','".$calle_secundario."','".$referencia."',2,'".mt_rand()."',now(),1,0)";
					if($DEBUG_STATUS)
						echo '$sql-1::'.$sql.'<br>';
					if(mysqli_query($dbcon,$sql))
		        	{
		        		$sql = "update z_viajescupon set nro_retornos_planificado=nro_retornos_planificado+".$nroasientessearch." where id_pago_ref='".$row["vp_codigo_pago_ref"]."'";
						if($DEBUG_STATUS)
							echo '$sql-2::'.$sql.'<br>';
				        if(mysqli_query($dbcon,$sql))
				        {
				        	$sql = "update z_viajesretornoplan set utilizado=1 where id='".$nroasientesid."'";
							if($DEBUG_STATUS)
								echo '$sql-2::'.$sql.'<br>';
					        if(mysqli_query($dbcon,$sql))
					        {
		...
		public function regsiterViajePlanificadoRetornoAeropuerto($dbcon,$sector,$nroasientessearch,$fechaviajesearch,$calle_principal,$numeracion,$calle_secundario,$referencia,$cedulas,$c_viaje,$DEBUG_STATUS)
		{
			mysqli_autocommit($dbcon,FALSE);
			$err_code=0;
			$ctr=0;
			...
			$sql="select vp_codigo_pago,vp_codigo_pago_ref,vc.nro_retornos_comprado,vc.valor_retorno_pagado from z_viajespasajero vp,z_viajescupon vc where vp.vp_id=".$c_viaje." and vp.vp_codigo_pago_ref=vc.id_pago_ref";
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result)) 
				{
					$sql = "insert into z_viajespasajero(vp_sector_inicio,vp_sector_destino,vp_codigo_pago,vp_codigo_pago_ref,vp_valor_total_pago,vp_nro_pasajeros,vp_comprado_por,vp_codigo_identification,vp_fecha_salida,vp_calle_principal,vp_numeracion,vp_calle_secundaria,vp_referencia,vp_estado_viaje,vp_codigo_confirmacion,vp_fecha_acceptation,vp_pago_verificado,vp_viaje_inicio) values 
								(".$sector.",1,'".$row["vp_codigo_pago"]."','".$row["vp_codigo_pago_ref"]."',".(($row["valor_retorno_pagado"]*$nroasientessearch)/$row["nro_retornos_comprado"]).",".$nroasientessearch.",".$_SESSION['userid'].",'".$cedulaStr."',DATE_FORMAT('".$fechaviajesearch."','%Y-%m-%d %H:%i:%s'),'".$calle_principal."','".$numeracion."','".$calle_secundario."','".$referencia."',2,'".mt_rand()."',now(),1,0)";
					if($DEBUG_STATUS)
						echo '$sql-1::'.$sql.'<br>';
					if(mysqli_query($dbcon,$sql))
		        	{
		...
		public function getDetallesPago($dbcon,$tipoPago,$temp_codigo_pago,$DEBUG_STATUS)
		{
			...
			return $detallesPago;*/
			$sql="select (vp.vp_valor_total_pago+valor_retorno_pagado) vp_valor_total_pago from z_viajespasajero vp,z_viajescupon vc where 
					vp.vp_codigo_pago_ref like '%$temp_codigo_pago%' and vp.vp_codigo_pago_ref=vc.id_pago_ref";
			$detallesPago=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
		...
		public function getUserMobileNumber($dbcon,$DEBUG_STATUS)
		{
			$usrCelular=0;
			if(isset($_SESSION['userid']))
			{
				$sql="select u_celular from z_users where u_id=".$_SESSION['userid'];
				$result = mysqli_query($dbcon,$sql);
	            if(mysqli_num_rows($result) > 0)  
	            {
					if($row = mysqli_fetch_assoc($result)) 
					{
						$usrCelular = $row["u_celular"];
					}
				}
			}
			return $usrCelular;
		...
		public function getPassengerListToPassenger($dbcon,$viaje_id,$DEBUG_STATUS)
		{
			$sql="select u.u_profile_pic,u.u_name,u.u_cedula,vp.vp_nro_pasajeros,u.u_celular,u.u_conventional,
			concat(vp.vp_calle_principal,' ',vp.vp_numeracion,' ',vp.vp_calle_secundaria) direccion,vp.vp_referencia,
			u.u_personal_interest,u.u_business_interest,u.u_id,
			(select estado from z_contactos where (u_id=".$_SESSION['userid']." and u_contactId=u.u_id) or (u_contactId=".$_SESSION['userid']." and u_id=u.u_id)) ctr
			from z_users u,z_viajespasajero vp where u.u_id!=".$_SESSION['userid']." and vp.vp_viaje_id=".$viaje_id." and vp.vp_comprado_por=u.u_id";
			//echo '$sql-1::'.$sql.'<br>';
			$passengerList=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					$passengerList[$count] = array(	$row["u_profile_pic"],
													...
													$row["direccion"],
													$row["vp_referencia"],
													$row["u_personal_interest"],
													$row["u_business_interest"],
													$row["u_id"],
													$row["ctr"]);
					$count++;
				}
		...
		public function addContact($dbcon,$contactId,$DEBUG_STATUS)
		{
			$err_code=1;
			$sql="select * from z_contactos where (u_id=".$_SESSION['userid']." and u_contactId=".$contactId.") or (u_contactId=".$_SESSION['userid']." and u_id=".$contactId.")";
			//echo $sql.'<br>';
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
            ...
            {
				$sql = "INSERT INTO z_contactos(u_id,u_contactId) values(".$_SESSION['userid'].",".$contactId.")";
				if($DEBUG_STATUS)
					echo '$sql-1::'.$sql.'<br>';
		        if(mysqli_query($dbcon,$sql))
		        {
		...
		public function aprobarContacto($dbcon,$solicitante,$DEBUG_STATUS)
		{
			$err_code=1;
			$sql = "UPDATE z_contactos set estado=2 where u_id=".$solicitante." and u_contactId=".$_SESSION['userid'];
			if($DEBUG_STATUS)
				echo '$sql-1::'.$sql.'<br>';
	        if(mysqli_query($dbcon,$sql))
	        {
		...
		public function getMiContactos($dbcon,$DEBUG_STATUS)
		{
			$err_code=1;
			$sql="	select u.u_id,c.u_contactId,u.u_profile_pic,u.u_name,u.u_personal_interest,u.u_business_interest,c.estado from z_contactos c,z_users u where c.u_id=".$_SESSION['userid']." and c.u_contactId=u.u_id
					union
					select u.u_id,c.u_contactId,u.u_profile_pic,u.u_name,u.u_personal_interest,u.u_business_interest,c.estado from z_contactos c,z_users u where c.u_contactId=".$_SESSION['userid']." and c.u_id=u.u_id";
		...
		public function registrarPago($dbcon,$tipoPago,$temp_codigo_pago,$nroDocPago,$tipoPago,$nombreArchivo,$fechaDocumentoPago,$montoDocumentoPago,$DEBUG_STATUS)
		{
			...
			$sqlPago = "select concat('".$nroDocPago."',date_format(now(),'%Y%m%d%H%i%s')) pago from dual";
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result)) 
				{
					...
					if($fechaDocumentoPago==null)
						$sql = "update z_viajespasajero set vp_codigo_pago='".$nroDocPago."',vp_codigo_pago_ref='".$strPagoId."',vp_fecha_pago_en_banco=now(),vp_tipo_pago='".$tipoPago."',vp_pic_pago='".$nombreArchivo."',vp_fecha_pago=now(),vp_estado_viaje=6,vp_pago_verificado=99 where vp_codigo_pago_ref like'%".$temp_codigo_pago."%'";
					else
						$sql = "update z_viajespasajero set vp_codigo_pago='".$nroDocPago."',vp_codigo_pago_ref='".$strPagoId."',vp_fecha_pago_en_banco=DATE_FORMAT('".$fechaDocumentoPago."','%Y-%m-%d'),vp_tipo_pago='".$tipoPago."',vp_pic_pago='".$nombreArchivo."',vp_fecha_pago=now(),vp_estado_viaje=6,vp_pago_verificado=99 where vp_codigo_pago_ref like'%".$temp_codigo_pago."%'";
					if($DEBUG_STATUS)
						echo '$sql-2::'.$sql.'<br>';
			        if(mysqli_query($dbcon,$sql))
			        {
			        	$sql = "update z_viajescupon set id_pago_ref='".$strPagoId."' where id_pago_ref like'%".$temp_codigo_pago."%'";
						if($DEBUG_STATUS)
							echo '$sql-2::'.$sql.'<br>';
				        if(mysqli_query($dbcon,$sql))
				        {
				        	$sql = "update z_viajesretornoplan set id_pago_ref='".$strPagoId."' where id_pago_ref like'%".$temp_codigo_pago."%'";
							if($DEBUG_STATUS)
								echo '$sql-2::'.$sql.'<br>';
					        if(mysqli_query($dbcon,$sql))
					        {
		...
		public function getConductorsViajesAsigandos($dbcon,$id_conductor,$hora_viaje,$from_airport,$DEBUG_STATUS)
		{
			$viajes=array();
			$count=0;
			//echo $id_conductor.'<br>';
			$conductor=explode(":",$id_conductor);
			/*if($from_airport==1)
			{
				$sql2="select v_viaje_id,s1.sector_name desde,s2.sector_name hasta,vp_nro_pasajeros,vp_fecha_salida from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
						where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2
						and vp.vp_sector_inicio=1  
						and z.v_id_conductor=".$conductor[0]." and 
						vp_estado_viaje=2 order by vp_fecha_salida desc";
				$result2 = mysqli_query($dbcon,$sql2);
				if(mysqli_num_rows($result2) > 0)  
	            {
					while($row2 = mysqli_fetch_assoc($result2)) 
					{
						$viajes[$count] = array($row2["v_viaje_id"],$row2["desde"],$row2["hasta"],$row2["vp_nro_pasajeros"],$row2["vp_fecha_salida"]);
						$count++;
					}
				}
			}
			else if($from_airport==0)
			{
				$sql2="select v_viaje_id,s1.sector_name desde,s2.sector_name hasta,vp_nro_pasajeros,vp_fecha_salida from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
						where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2
						and vp.vp_sector_destino=1  
						and z.v_id_conductor=".$conductor[0]." and 
						vp_estado_viaje=2 order by vp_fecha_salida desc";
				//echo $sql2;
				$result2 = mysqli_query($dbcon,$sql2);
			...
			$sql2="select v_viaje_id,s1.sector_name desde,s2.sector_name hasta,vp_nro_pasajeros,vp_fecha_salida from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
						where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2
						and z.v_id_conductor=".$conductor[0]." and 
						date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')>=DATE_SUB(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 2880 MINUTE) and
						date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')<=DATE_ADD(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 2880 MINUTE) and 										
						vp_estado_viaje=2 order by vp_fecha_salida desc";
				$result2 = mysqli_query($dbcon,$sql2);
				if(mysqli_num_rows($result2) > 0)  
	            {
		...
		{
			$sql = "select a.u_id,a.u_name from (
				...
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					if($from_airport==1)
					{
						$sql2="select 3-sum(vp.vp_nro_pasajeros) ctr,v_viaje_id from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
								where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2
								and vp.vp_sector_inicio=1  
								and z.v_id_conductor=".$row["u_id"]." and date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')=DATE_SUB(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 0 MINUTE) 	group by z.v_id_conductor";
						$result2 = mysqli_query($dbcon,$sql2);
						if(mysqli_num_rows($result2) > 0)  
			            {
					...
					/*if($from_airport==1)
					{
		            	...
		            	{
		            		$sql3="select 3-sum(vp.vp_nro_pasajeros) ctr,v_viaje_id from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
								where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2 
								and vp.vp_sector_destino=1 
								and z.v_id_conductor=".$row["u_id"]." and date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')=DATE_SUB(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 90 MINUTE) 	group by z.v_id_conductor";
							...
							{
								$sql4 = "select 3-sum(vp.vp_nro_pasajeros) ctr,v_viaje_id from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
								where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2 
								and vp.vp_sector_destino=1 
								and z.v_id_conductor=".$row["u_id"]." and date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')=DATE_ADD(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 90 MINUTE) 	group by z.v_id_conductor";
								...
								{									
									$sql5 = "select vp.* from z_viajespasajero vp,z_viajes z where vp.vp_viaje_id=z.v_viaje_id and
										date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')>DATE_SUB(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 180 MINUTE) and
										date_format(vp.vp_fecha_salida,'%Y-%m-%d %H:%i:%s')<DATE_ADD(date_format('".$hora_viaje."','%Y%m%d%H%i%s'),INTERVAL 180 MINUTE) and 
		...
		public function getConductorsForceAssign($dbcon,$hora_viaje,$from_airport,$DEBUG_STATUS)
		{
			$sql = "select * from (
					select distinct u.u_id,u.u_name from z_users u,z_userdocs ud where u_is_email_verified=0 
					and ((u_celular is not null and u_celular!='') or (u_conventional is not null and u_conventional!=''))
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				...
				{					
						...
						/*$sql2="select 3-sum(vp.vp_nro_pasajeros) ctr,v_viaje_id from z_viajespasajero vp, z_sector s1,z_sector s2,z_viajes z 
							where z.v_viaje_id=vp.vp_viaje_id and vp.vp_sector_inicio=s1.id and vp.vp_sector_destino=s2.id and vp.vp_estado_viaje=2 							
							and z.v_id_conductor=".$row["u_id"]." and date_format(vp.vp_fecha_salida,'%Y%m%d%h%i%s')=date_format('".$hora_viaje."','%Y%m%d%h%i%s') 	
							group by z.v_id_conductor";*/
						$sql2 = "select 3-sum(vp.vp_nro_pasajeros) ctr,v_viaje_id from z_viajespasajero vp,z_viajes z where vp.vp_viaje_id=z.v_viaje_id and
									date_format(vp.vp_fecha_salida,'%Y-%m-%d %h:%i:%s')>DATE_SUB(date_format('".$hora_viaje."','%Y%m%d%h%i%s'),INTERVAL 180 MINUTE) and
									date_format(vp.vp_fecha_salida,'%Y-%m-%d %h:%i:%s')<DATE_ADD(date_format('".$hora_viaje."','%Y%m%d%h%i%s'),INTERVAL 180 MINUTE) and 
									z.v_id_conductor=".$row["u_id"]." and vp.vp_estado_viaje in (2,3,4)  group by z.v_id_conductor";
						$result2 = mysqli_query($dbcon,$sql2);
						if(mysqli_num_rows($result2) == 0)  
			            {
							$conductors[$count] = array($row["u_id"],$row["u_name"],$capacity,$codigo_viaje);
						...
						else if($from_airport==1)
						{
							$sql2 = "select vp.* from z_viajespasajero vp,z_viajes z where vp.vp_viaje_id=z.v_viaje_id and
									date_format(vp.vp_fecha_salida,'%Y-%m-%d %h:%i:%s')=DATE_SUB(date_format('".$hora_viaje."','%Y%m%d%h%i%s'),INTERVAL 0 MINUTE) and
									vp.vp_sector_inicio=1 and 
									z.v_id_conductor=".$row["u_id"]." and vp.vp_estado_viaje in (2,3,4)";
							$result2 = mysqli_query($dbcon,$sql2);
							if(mysqli_num_rows($result2) > 0)  
				            {
		...
		public function asientesRetornoPendientes($dbcon,$codigoIda,$DEBUG_STATUS)
		{
			/*$sql="select (vc.nro_retornos_comprado - vc.nro_retornos_planificado) retornos_pendientes,date_format(vp_fecha_salida,'%Y-%m-%d') vp_fecha_salida,vc.codigo_retorno from z_viajespasajero vp,z_viajescupon vc
					where vp.vp_id=".$codigoIda." and vp.vp_codigo_pago_ref=vc.id_pago_ref";
			$retornos_pendientes=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result))
				{
					$retornos_pendientes[$count] = array($row["retornos_pendientes"],$row["vp_fecha_salida"],$row["codigo_retorno"]);
					$count++;
				}
			}
			return $retornos_pendientes;*/
			$sql="select vr.nro_asientos retornos_pendientes,date_format(vp_fecha_salida,'%Y-%m-%d') vp_fecha_salida,vr.id,vc.codigo_retorno from z_viajespasajero vp,z_viajescupon vc,z_viajesretornoplan vr
					where vp.vp_id=".$codigoIda." and vp.vp_codigo_pago_ref=vc.id_pago_ref and vc.id_pago_ref=vr.id_pago_ref and vr.utilizado=0 order by vr.nro_asientos";
			$retornos_pendientes=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result))
				{
					$retornos_pendientes[$count] = array($row["retornos_pendientes"],$row["vp_fecha_salida"],$row["id"],$row["codigo_retorno"]);
					$count++;
				}
			}
			return $retornos_pendientes;
		...
		public function getNroAsientosSearch($dbcon,$nroAsientosSearch,$DEBUG_STATUS)
		{
			$sql="select vr.nro_asientos from z_viajesretornoplan vr where vr.id=".$nroAsientosSearch;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result))
				{
					$nro_asientos =$row["nro_asientos"];
				}
			}
			return $nro_asientos;
		}
