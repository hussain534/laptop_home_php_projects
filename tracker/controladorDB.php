...

	{
...

		{
			...
			if($id!=0)
				...
				$sql="SELECT l.id,l.nombre,l.email,p.nombre perfil,l.telefono,l.celular,l.ubicacion,l.id_paralelo,l.habilitado,(select nombre from c_paralelo cp where cp.id=l.id_paralelo) nombre_paralelo FROM c_login l, c_perfil p WHERE l.perfil=p.id AND l.habilitado=1 AND p.habilitado=1 and l.id=".$id;

			else

				$sql="SELECT l.id,l.nombre,l.email,p.nombre perfil,l.telefono,l.celular,l.ubicacion,l.id_paralelo,l.habilitado,(select nombre from c_paralelo cp where cp.id=l.id_paralelo) nombre_paralelo FROM c_login l, c_perfil p WHERE l.perfil=p.id AND l.habilitado=1 AND p.habilitado=1";
...

            {
...

				{
					...
					$users[$count] = array($row["id"],$row["nombre"],$row["email"],$row["perfil"],$row["telefono"],$row["celular"],$row["ubicacion"],$row["habilitado"],$row["id_paralelo"],$row["nombre_paralelo"]);
