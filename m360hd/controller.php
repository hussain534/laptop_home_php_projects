<?php
    ...
    if(isset($_GET["controller"]))
    {
        ...
        {
            //OK
            $err = $controller->registerUser($databasecon,$_POST["perfil"],$_POST["userEmail"],$_POST["userName"],$_POST["userPwd"],$DEBUG_STATUS);    
            //echo $err.'<br>';
        ...
        else if($_GET["controller"]==1 && $_GET["task"]==0)
        {
            ...
            $err = $controller->registerNewTask($databasecon,$_POST["projectCode"],$_POST["id_ambiente"],$_POST["aprobacion_Cliente"],$_POST["categorea_tarea"],$_POST["taskDate"],$_POST["horaInicioTarea"],$_POST["horaFinTarea"],$_POST["tiempo_indisponibilidad"],$_POST["descBreveTarea"],$_POST["descCompTarea"],$data[1],$_POST["base_conocimiento"],$DEBUG_STATUS);    
        ...
        {
            //ok
            $tasks = $controller->getKDBSearch($databasecon,1,$_POST["baseconocimiento"],$DEBUG_STATUS);    
            //echo $err.'<br>';
            $_SESSION["tasks"]=$tasks;
            $nextView='baseconocimiento.php';
            header("Location:$nextView");
        }
        else if($_GET["controller"]==3 && $_GET["task"]==0)
        {
            ...
            //echo 'File Upload result::'.$data[0].'<br>';
            //echo 'File Path::'.$data[1].'<br>';
            $err = $controller->assignNewTask($databasecon,$_POST["tipoTarea"],$_POST["notificadoPorMedio"],$_POST["notificadoPor"],$_POST["prioridad"],$_POST["fechaNotificacion"],$_POST["horaNotificacion"],$_POST["id_tecnico"],$_POST["servicioAppl"],$_POST["descBreveTarea"],$_POST["descCompTarea"],$data[1],$_POST["listaServidores"],$DEBUG_STATUS);    
            $_SESSION["err_code"]=$err;
            $nextView='asignarTrabajo.php';
            header("Location:$nextView");
        }
        else if($_GET["controller"]==3 && $_GET["task"]==1)
        {
            //ok
            $tasks = $controller->updateNotidacionDone($databasecon,$_GET["medio"],$_GET["id"],$DEBUG_STATUS);    
            //echo $err.'<br>';
            //$_SESSION["tasks"]=$tasks;
            //$nextView='asignarTrabajo.php';
            $nextView=$_SESSION["last_url"];
            header("Location:$nextView");
        }
        else if($_GET["controller"]==3 && $_GET["task"]==3)
        {
            $_SESSION["codProyecto"]=$_POST["projectCode"];
            $_SESSION["fechaInicio"]=$_POST["taskDateInicial"];
            $_SESSION["fechaFin"]=$_POST["taskDateFinal"];
            $_SESSION["user"]=$_POST["taskOwner"];
            $tasks = $controller->getAssignedTaskDetailBySearchParam($databasecon,1,$_POST["projectCode"],$_POST["id_ambiente"],$_POST["taskDateInicial"],$_POST["taskDateFinal"],$_POST["taskOwner"],$_POST["categorea_tarea"],$DEBUG_STATUS);    
            //echo $err.'<br>';
            $_SESSION["tasks"]=$tasks;
            $nextView='mostrarTrabajosAsignados.php';
            header("Location:$nextView");
        }
        else if($_GET["controller"]==3 && $_GET["task"]==4)
        {
            //ok
            if(isset($_FILES["fileToUpload"]["name"]))
            {
            ...
            //echo 'File Upload result::'.$data[0].'<br>';
            //echo 'File Path::'.$data[1].'<br>';
            $err = $controller->registrarGestion($databasecon,$_POST["tid"],$_POST["estadoTarea"],$_POST["id_tecnico_new"],$_POST["tecnologiaProducto"],$_POST["solucionAplicado"],$_POST["causaProblema"],$_POST["impactProblema"],$_POST["comentarios"],$data[1],$_POST["base_conocimiento"],$DEBUG_STATUS);    
            $_SESSION["err_code"]=$err;
            $nextView='registrarGestion.php?tid='.$_POST["tid"];
            header("Location:$nextView");
        }
        else if($_GET["controller"]==4 && $_GET["task"]==0)
        {
            //OK
            $tasks = $controller->getSearchData($databasecon,$_POST["searchParam"],$DEBUG_STATUS);    
            //echo $err.'<br>';
            $_SESSION["tasks"]=$tasks;
            $nextView='searchData.php';
            header("Location:$nextView");
        }
        else
        {
