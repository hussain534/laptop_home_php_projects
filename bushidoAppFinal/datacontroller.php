<?php
	...
	if(isset($_GET["dojob"]))
	{
		...
		{	
			...
			{
				$url='index.php';
				$_SESSION["message"]="Usuario no existe o clave invalida";
			}
			header("Location:$url");
		}
		...
		{	
			$err = $controller->deleteDoc($databasecon,$_GET['id_doc'],$DEBUG_STATUS);	
			echo 'FINAL:'.$err.'<br>';
			if($err==0)
			{
				$_SESSION["message"]="<center>ERROR EN ELIMINAR DOCUMENTO. INTENTA MAS TARDE</center>";
			}
			else
			{
				$_SESSION["message"]="<center>DOCUMENTO ELIMINADO CORRECTAMENTE.</center>";	
			}
		...
		{	
			$err = $controller->deletePuntos($databasecon,$_GET['id_puntos'],$DEBUG_STATUS);	
			echo 'FINAL:'.$err.'<br>';
			if($err==0)
			{
				$_SESSION["message"]="<center>ERROR EN ELIMINAR PUNTOS. INTENTA MAS TARDE</center>";
			}
			else
			{
				$_SESSION["message"]="<center>PUNTOS ELIMINADO CORRECTAMENTE.</center>";	
			}
		...
		{	
			$err = $controller->deleteUsuario($databasecon,$_GET['user'],$DEBUG_STATUS);	
			echo 'FINAL:'.$err.'<br>';
			if($err==0)
			{
				$_SESSION["message"]="<center>ERROR EN ELIMINAR USUARIO. INTENTA MAS TARDE</center>";
			}
			else
			{
				$_SESSION["message"]="<center>USUARIo ELIMINADO CORRECTAMENTE.</center>";	
			}
