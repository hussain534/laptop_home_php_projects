...
if(session_status() == PHP_SESSION_NONE)
	...
	class controller
	{
		...
		public function gestionInventoryItems($dbcon,$item_id,$item_name,$item_category,$item_desc,$item_qty,$item_ppu,$item_iva,$item_prov_ruc,$item_accion,$fecha_expiracion,$barcode,$purchase_price,$consumption_type,$alert_store,$alert_godown1,$alert_godown2,$DEBUG_STATUS)
		{
            ...
            if($item_accion == 0)
            {
            	$sql = "INSERT INTO ct_item(name,id_category, detail,quantity,price_per_unit,iva,id_provider,client_id,fecha_expiracion,enabled,bar_code,purchase_price,consumption_type,alert_store,alert_godown1,alert_godown2) 
				values('".$item_name."',".$item_category.",'".$item_desc."',".$item_qty.",".$item_ppu.",".$item_iva.",".$item_prov_ruc.",".$_SESSION["user_client_id"].",date_format('".$fecha_expiracion."','%Y-%m-%d'),1,'".$barcode."',".$purchase_price.",".$consumption_type.",".$alert_store.",".$alert_godown1.",".$alert_godown2.")";
				echo $sql.'<br>';
		        if(mysqli_query($dbcon,$sql))
			    {
			    	mysqli_commit($dbcon);
            ...
            else
            {
            	$sql="UPDATE ct_item set name='".$item_name."',id_category=".$item_category.", detail='".$item_desc."',quantity=".$item_qty.",
            				price_per_unit=".$item_ppu.",iva=".$item_iva.",id_provider=".$item_prov_ruc.", client_id=".$_SESSION["user_client_id"].",fecha_expiracion=date_format('".$fecha_expiracion."','%Y-%m-%d'), 
            				bar_code='".$barcode."',purchase_price=".$purchase_price.",consumption_type=".$consumption_type.",alert_store=".$alert_store.",alert_godown1=".$alert_godown1.",alert_godown2=".$alert_godown2." 
            				where id=".$item_id;
            	//echo $sql;
            	if(mysqli_query($dbcon,$sql))
		        {
		...
		public function getInventoryItems($dbcon,$DEBUG_STATUS)
		{
			...
			$sql="select id, name,detail,id_category,(select ic.name from ct_item_category ic where ic.id=id_category) desc_category,
				...
				bar_code,purchase_price,consumption_type,alert_store,alert_godown1,alert_godown2 from ct_item ci where client_id=".$_SESSION["user_client_id"];
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$branches[$count] = array(	$row["id"],$row["name"],$row["detail"],$row["id_category"],$row["desc_category"]
												,$row["quantity"],$row["price_per_unit"],$row["iva"],$row["id_provider"],
												$row["name_provider"],$row["fecha_expiracion"],$row["enabled"],
