	...
	if(isset($_GET["dojob"]))
	{
		...
		*/
		...
		else if($_GET["dojob"]==8 && $_GET["metodo"]==0)
		{
			...
			{
				if($err==2)
					$_SESSION["message"]='SALA EXISTE Y HABILITADO EN BASE.';
				else	
					$_SESSION["message"]='SALA GUARDADO CORRECTAMENTE';
			}
			else
				$_SESSION["message"]='ERROR EN CREACION DE SALA';
		}
		...
		{
			//echo '20-<br>';
			$err = $controller->addUser($databasecon,$_GET["supervision_id"],$_GET["user_name"],$_GET["user_email"],$_GET["user_tele"],$_GET["user_celular"],$_GET["user_direccion"],$_GET["perfil_id"],$_GET["client_id"],$_GET["ciudad_id"],$_GET["sucursal_id"],$_GET["sala_id"],$DEBUG_STATUS);	
			echo $err.'<br>';
			if($err>0)	
			{
				if($err==2)
		...
		{
			$err = $controller->editUser($databasecon,$_GET["user_id"],$_GET["supervision_id"],$_GET["user_name"],$_GET["user_email"],$_GET["user_tele"],$_GET["user_celular"],$_GET["user_direccion"],$_GET["perfil_id"],$_GET["client_id"],$_GET["ciudad_id"],$_GET["sucursal_id"],$_GET["sala_id"],$DEBUG_STATUS);	
			echo $err.'<br>';
			if($err>0)	
			{
		...
		{
			$users = $controller->buscarUser($databasecon,$_GET["user_name"],$_GET["user_email"],$_GET["user_tele"],$_GET["user_celular"],$_GET["user_direccion"],$_GET["perfil_id"],$_GET["client_id"],$_GET["ciudad_id"],$_GET["sucursal_id"],$_GET["sala_id"],$DEBUG_STATUS);	
			if(isset($users))
