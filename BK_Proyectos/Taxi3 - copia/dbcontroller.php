<?php
	...
	class controller
	{
		public function doRegisterAndLogin($dbcon,$userEmail,$userPwd,$userRole,$DEBUG_STATUS)
		{
	        ...
	        if(mysqli_query($dbcon,$sql))
	        {
	            ...
	            if(mysqli_query($dbcon,$sql))
	            {
					...
					$txt = '¡HOLA!'."\n\n\n";
					$txt=$txt.'Se ha registrado cuenta en ZIELUS'."\n\n\n";
					$txt=$txt.'Usa la dirección de correo electrónico '.$userEmail.' y tuclave ingresada el momento del registro para iniciar sesión.'."\n\n\n";
					$txt=$txt.'Para la activacion del cuenta , por favor haz clic en la siguiente link.'."\n\n\n";
					$txt=$txt.'LINK DE ACTIVACION:http://localhost/Taxi3/activarCuenta.php?id='.$confirmacion_codigo."\n\n\n";
					$txt=$txt.'En caso de tener problemas, puedes copiarle ese link y pegarle en la barra de direccion en cualquier navegador de tu gusto.'."\n\n\n";
					$txt=$txt.'Si tienes alguna pregunta o necesitas ayuda, puedes ponerte en contacto con nosotros en support@zielus.com'."\n\n\n";
					$txt=$txt.'¡Disfruta de esta herramienta creada para ti!'."\n\n\n";
					$txt=$txt.'El Equipo de Zielus - Ecuador'."\n\n\n";
		...
		{
			$sql="select * from z_viajes_solicitado where vs_estado_solicitud=0 and vs_desde=5 and vs_hasta=1 order by vs_created_on desc";
			$misreservas=array();
			$count=0;
			//$cant_retorno_pendiente = 0;
			$result = mysqli_query($dbcon,$sql);
		...
		public function consultapagos($dbcon,$userId,$DEBUG_STATUS)
		{
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$misconsultas[$count] = array($row["v_id"],$row["desde"],$row["hasta"],$row["v_fecha_salida"],$row["v_estado"],$row["v_costo_viaje"],$row["vp_tipo_pago"],$row["vp_fecha_aceptacion"],$row["nombre_pasajero"],$row["vp_estado_viaje"],$row["vp_fecha_pago"]);
					$count++;
				}
			}
			return $misconsultas;
		...
		public function controlPagos($dbcon,$DEBUG_STATUS)
		{
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					$docs[$count] = array($row["vp_id"],$row["vp_codigo_viaje"],$row["vp_fecha_pago"],$strPago,$row["vp_pic_pago"],$row["u_name"],$row["vp_observacion"],$row["vp_pago_verificado"]);
					$count++;
				}
			}
			return $docs;
