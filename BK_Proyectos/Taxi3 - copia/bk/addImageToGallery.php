<?php
	...
	if(isset($_POST['submitted']))
    {
        $img_caption=$_POST['img_caption'];
            ...
            $viajeIDGallery=$_POST['viajeIDGallery'];
        if($DEBUG_STATUS)
        {
        ...
        if(basename($_FILES["fileToUpload"]["name"]))
        {
            ...
            $target_file = $target_dir .'Gallery-'.$viajeIDGallery.'-'.mt_rand().'.jpg';
            $uploadOk = 1;
        ...
        else
        {
            ...
            $updStatus = $controller->insertGalleryImage($databasecon,$viajeIDGallery,$_SESSION['userid'],$img_caption,$img_desc,$target_file,$DEBUG_STATUS);
            
        	if($DEBUG_STATUS)
                echo $updStatus.'<br>';
	        if($updStatus==0)
	        {
	            $_SESSION["session_msg"]= "Imagen the viaje guardado correctamente";
	        }
	        else
	        {
	        	$_SESSION["session_msg"]= 'Error en guardar el imagen. Por favor intenta nuevamente';
