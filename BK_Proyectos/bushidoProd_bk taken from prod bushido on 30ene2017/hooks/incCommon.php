<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			'reglamento' => array('Lineamientos del Bushido', '', 'resources/table_icons/apple-icon-72x72.png', 'Lineamientos Generales Bushido'),
			'puntos' => array('Mi Puntaje', 'Aqu&#237; se registran los puntos ganados por los colaboradores .<br>', 'resources/table_icons/dashboard.png', 'Camino del Samurai'),
			'impactos' => array('Impactos', 'Se detalla los tipos de puntajes que se podran adquirir a lo largo del programa.', 'resources/table_icons/binary-thinking.png', 'Lineamientos Generales Bushido'),
			'persona' => array('Persona', '', 'resources/table_icons/business-male-team.png', 'RRHH')
		);
	...
	function get_table_groups($skip_authentication = false){
		...
		$all_groups = array('Lineamientos Generales Bushido', 'Camino del Samurai', 'RRHH');
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'reglamento' => "`reglamento`.`id` as 'id', `reglamento`.`archivo` as 'archivo', `reglamento`.`descripcion` as 'descripcion'",
			'puntos' => "`puntos`.`id` as 'id', if(`puntos`.`fecha_actividad`,date_format(`puntos`.`fecha_actividad`,'%m/%d/%Y'),'') as 'fecha_actividad', `puntos`.`fecha_ingreso` as 'fecha_ingreso', `puntos`.`motivo` as 'motivo', `puntos`.`puntaje` as 'puntaje', IF(    CHAR_LENGTH(`impactos1`.`impactos`), CONCAT_WS('',   `impactos1`.`impactos`), '') as 'impacto', IF(    CHAR_LENGTH(`impactos1`.`imagen`), CONCAT_WS('',   `impactos1`.`imagen`), '') as 'emblema', IF(    CHAR_LENGTH(`persona1`.`nombre`) || CHAR_LENGTH(`persona1`.`apellido`), CONCAT_WS('',   `persona1`.`nombre`, ' ', `persona1`.`apellido`), '') as 'postulante', IF(    CHAR_LENGTH(`persona2`.`usuario`), CONCAT_WS('',   `persona2`.`usuario`, ' '), '') as 'persona'",
			'impactos' => "`impactos`.`id` as 'id', `impactos`.`puntaje` as 'puntaje', `impactos`.`impactos` as 'impactos', `impactos`.`imagen` as 'imagen'",
			'persona' => "`persona`.`id` as 'id', `persona`.`usuario` as 'usuario', `persona`.`nombre` as 'nombre', `persona`.`apellido` as 'apellido', `persona`.`correo` as 'correo', `persona`.`celular` as 'celular', if(`persona`.`cumpleanos`,date_format(`persona`.`cumpleanos`,'%m/%d/%Y'),'') as 'cumpleanos'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'puntos' => "`puntos` LEFT JOIN `impactos` as impactos1 ON `impactos1`.`id`=`puntos`.`impacto` LEFT JOIN `persona` as persona1 ON `persona1`.`id`=`puntos`.`postulante` LEFT JOIN `persona` as persona2 ON `persona2`.`id`=`puntos`.`persona` ",
			'impactos' => "`impactos` ",
			'persona' => "`persona` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('bushidoapp_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('bushidoapp_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['bushidoapp_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['bushidoapp_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> bushidoapp</a>
			</div>
	...
	function NavMenus($options = array()){
		...
		if(is_array($arrTables)){
			foreach($arrTables as $tn => $tc){
				...
				$tChkHL = array_search($tn, array('puntos','impactos'));
