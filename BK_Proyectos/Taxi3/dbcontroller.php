	...
	class controller
	{
		...
		public function getVehicleDetails($dbcon,$userId,$DEBUG_STATUS)
		{
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$vehDtl[$count] = array($row["a_id"],$row["a_marca"],$row["a_modelo"],$row["a_ano"],$row["a_capacidad"],$row["a_placa"],$row["a_nro_matricula"],$row["a_pic_automovil"],$row["a_pic_matriculation"],$row["a_docId"],$row["d_is_doc_verified"],$row["a_is_approved"]);
					$count++;
				}
			}
			return $vehDtl;
		...
		public function misreservas($dbcon,$userId,$DEBUG_STATUS)
		{
			$sql="select vp.vp_codigo_viaje v_codigo_viaje,t1.t_name desde,t2.t_name hasta,
				DATE_FORMAT(v.v_fecha_salida,'%d-%b-%Y %H:%i:%s') v_fecha_salida,
				(case when v.v_fecha_salida<now() then 1 else 0 end) open_terminate,
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					if($row["vp_estado_viaje"]==0)
						$misreservas[$count] = array($row["v_codigo_viaje"],$row["desde"],$row["hasta"],'',$row["v_estado"],$row["v_costo_viaje"],$row["vp_tipo_pago"],$row["vp_fecha_aceptacion"],$row["vp_viaje_id"],$row["open_terminate"]);
					else
						$misreservas[$count] = array($row["v_codigo_viaje"],$row["desde"],$row["hasta"],$row["v_fecha_salida"],$row["v_estado"],$row["v_costo_viaje"],$row["vp_tipo_pago"],$row["vp_fecha_aceptacion"],$row["vp_viaje_id"],$row["open_terminate"]);
					$count++;
				}
			}
			return $misreservas;
		...
		public function consultapagos($dbcon,$userId,$DEBUG_STATUS)
		{
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$misconsultas[$count] = array($row["v_id"],$row["desde"],$row["hasta"],$row["v_fecha_salida"],$row["v_estado"],$row["v_costo_viaje"],$row["vp_tipo_pago"],$row["vp_fecha_aceptacion"],$row["nombre_pasajero"],$row["vp_estado_viaje"],$row["vp_dt_payment_to_client"],$row["vp_doc_paymet_to_client"]);
					$count++;
				}
			}
			return $misconsultas;
		...
		public function mispublicaciones($dbcon,$userId,$DEBUG_STATUS)
		{
			...
			$sql="select * from (
					select v.v_id,v.v_codigo_viaje,t1.t_name desde,t2.t_name hasta,DATE_FORMAT(v.v_fecha_salida,'%d-%b-%Y %H:%i:%s') v_fecha_salida,
					(select p_desc from z_parametros where p_id=v.v_estado) v_estado,v.v_costo_viaje,v.v_min_pasajeros,v.v_ocupado,
					DATE_FORMAT(v.v_created_on,'%d-%b-%Y %H:%i:%s') v_created_on,v.v_fecha_salida fecha_viaje from z_viajes v,z_users u,z_terminal t1,z_terminal t2 where v.v_created_by=u.u_id and 
					v.v_desde=t1.t_id and v.v_hasta=t2.t_id and v.v_created_by=".$userId." and v.v_estado in(0,1,2,3,4,5,6,7) and v.v_fecha_salida>=now()
					union 
					select v.v_id,v.v_codigo_viaje,t1.t_name desde,t2.t_name hasta,DATE_FORMAT(v.v_fecha_salida,'%d-%b-%Y %H:%i:%s') v_fecha_salida,
					(case when v_estado in(0,1,5) then 'CANCELADO' when v_estado=2 then 'PROGRAMADO' end) v_estado,v.v_costo_viaje,v.v_min_pasajeros,v.v_ocupado,
					DATE_FORMAT(v.v_created_on,'%d-%b-%Y %H:%i:%s') v_created_on,v.v_fecha_salida fecha_viaje from z_viajes v,z_users u,z_terminal t1,z_terminal t2 where v.v_created_by=u.u_id and 
					v.v_desde=t1.t_id and v.v_hasta=t2.t_id and v.v_created_by=".$userId." and v.v_estado in(0,1,2,5) and v.v_fecha_salida<now()) a order by a.fecha_viaje";
			$misreservas=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					if(isset($row["v_estado"]) && !empty($row["v_estado"]))
						...
						$misreservas[$count] = array($row["v_codigo_viaje"],$row["desde"],$row["hasta"],$row["v_fecha_salida"],'PUBLICADO',$row["v_costo_viaje"],$row["v_min_pasajeros"],$row["v_ocupado"],$row["v_created_on"],$row["v_id"]);
					$count++;
				}
			}
			return $misreservas;
		...
		public function publicacionesConductor($dbcon,$userId,$estado,$DEBUG_STATUS)
		{
			if($estado==1)
			{
				$sql="select v.v_id,u.u_name,t1.t_name desde,t2.t_name hasta,DATE_FORMAT(v.v_fecha_salida,'%d-%b-%Y %H:%i:%s') v_fecha_salida,
				(select p_desc from z_parametros where p_id=v_estado) v_estado,v.v_costo_viaje,v.v_min_pasajeros,v.v_ocupado,
				DATE_FORMAT(v.v_created_on,'%d-%b-%Y %H:%i:%s') v_created_on,v.v_created_by,(case when v.v_fecha_salida<now() then 0 else 1 end) viaje_rel 
				from z_viajes v,z_users u,z_terminal t1,z_terminal t2 where v.v_created_by=u.u_id and 
				v.v_desde=t1.t_id and v.v_hasta=t2.t_id";
				if($userId!=0)
					$sql=$sql." and v.v_created_by=".$userId;
				
				$sql=$sql." and v_estado<=".$estado;

				$sql=$sql." order by v.v_fecha_salida";
				$misreservas=array();
				$count=0;
				$result = mysqli_query($dbcon,$sql);
	            if(mysqli_num_rows($result) > 0)  
	            {
					...
					{
						if($row["viaje_rel"]==1)
							$misreservas[$count] = array($row["v_created_by"].':'.strtoupper($row["u_name"]),$row["desde"],$row["hasta"],$row["v_fecha_salida"],'PUBLICADO',$row["v_costo_viaje"],$row["v_min_pasajeros"],$row["v_ocupado"],$row["v_created_on"],$row["v_id"]);
						else
							$misreservas[$count] = array($row["v_created_by"].':'.strtoupper($row["u_name"]),$row["desde"],$row["hasta"],$row["v_fecha_salida"],'CANCELADO',$row["v_costo_viaje"],$row["v_min_pasajeros"],$row["v_ocupado"],$row["v_created_on"],$row["v_id"]);
						$count++;
					}
				}
			}
			else if($estado>1 && $estado!=99)
			{
				$sql="select v.v_id,u.u_name,t1.t_name desde,t2.t_name hasta,DATE_FORMAT(v.v_fecha_salida,'%d-%b-%Y %H:%i:%s') v_fecha_salida,
				(select p_desc from z_parametros where p_id=vp.vp_estado_viaje) vp_estado_viaje,v.v_costo_viaje,v.v_min_pasajeros,v.v_ocupado,
				DATE_FORMAT(v.v_created_on,'%d-%b-%Y %H:%i:%s') v_created_on,v.v_created_by,vp.vp_id  
				from z_viajes v,z_users u,z_terminal t1,z_terminal t2,z_viajespasajero vp where v.v_created_by=u.u_id and 
				v.v_id=vp.vp_viaje_id and v.v_desde=t1.t_id and v.v_hasta=t2.t_id";
				if($userId!=0)
					$sql=$sql." and v.v_created_by=".$userId;
				if($estado!=99)
					$sql=$sql." and vp.vp_estado_viaje=".$estado;
				$sql=$sql." order by v.v_fecha_salida";
				$misreservas=array();
				$count=0;
				$result = mysqli_query($dbcon,$sql);
	            if(mysqli_num_rows($result) > 0)  
	            {
					while($row = mysqli_fetch_assoc($result)) 
					{
						if(isset($row["vp_estado_viaje"]) && !empty($row["vp_estado_viaje"]))
							$misreservas[$count] = array($row["v_created_by"].':'.strtoupper($row["u_name"]),$row["desde"],$row["hasta"],$row["v_fecha_salida"],$row["vp_estado_viaje"],$row["v_costo_viaje"],$row["v_min_pasajeros"],$row["v_ocupado"],$row["v_created_on"],$row["v_id"],$row["vp_id"]);
						else
							$misreservas[$count] = array($row["v_created_by"].':'.strtoupper($row["u_name"]),$row["desde"],$row["hasta"],$row["v_fecha_salida"],'PUBLICADO',$row["v_costo_viaje"],$row["v_min_pasajeros"],$row["v_ocupado"],$row["v_created_on"],$row["v_id"],$row["vp_id"]);
						$count++;
					}
				}
			}
			else
			{
				...
				if($userId!=0)
					$sql=$sql." and v.v_created_by=".$userId;
				$sql=$sql." and vp.vp_estado_viaje>1 ";
				$sql=$sql." order by v.v_fecha_salida";
				//$misreservas=array();
				//$count=0;
				$result = mysqli_query($dbcon,$sql);
		...
		{
			$sql="select vp.vp_viaje_id,c.c_userid,c.c_nro_cuenta,b.b_name,t.tc_desc,v.v_costo_viaje,vp.vp_doc_paymet_to_client,vp_dt_payment_to_client    
				from z_viajespasajero vp,z_viajes v, z_cuenta c, z_bancos b,z_tipo_cuenta t
				where vp.vp_id=".$viajeId." and vp.vp_viaje_id=v.v_id and v.v_created_by=c.c_userid and c.c_banco_id=b.b_id and c.c_tipo_cuenta=t.tc_id";
			$micuenta=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$micuenta[$count] = array($row["vp_viaje_id"],$row["c_userid"],$row["c_nro_cuenta"],$row["b_name"],$row["tc_desc"],$row["v_costo_viaje"],$row["vp_doc_paymet_to_client"],$row["vp_dt_payment_to_client"]);
					$count++;
				}
			}
			return $micuenta;
		...
		public function controlPagos($dbcon,$DEBUG_STATUS)
		{
			...
			$sql="select distinct a.vp_codigo_viaje,a.vp_fecha_pago,a.vp_tipo_pago,a.vp_pic_pago,a.u_name,a.vp_observacion,a.vp_pago_verificado,
					...
					vp.vp_pago_verificado,v.v_costo_viaje from z_viajespasajero vp,z_users u,z_viajes v where vp.vp_pago_verificado=99 and 
					vp.vp_pass_id=u.u_id and vp.vp_viaje_id=v.v_id order by vp.vp_fecha_pago) a 
					group by a.vp_codigo_viaje,a.vp_fecha_pago,a.vp_tipo_pago,a.vp_pic_pago,a.u_name,a.vp_observacion,a.vp_pago_verificado";
			$docs=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					...
					$docs[$count] = array(1,$row["vp_codigo_viaje"],$row["vp_fecha_pago"],$strPago,$row["vp_pic_pago"],$row["u_name"],$row["vp_observacion"],$row["vp_pago_verificado"],$row["v_costo_viaje"]);
					$count++;
				}
			}
			return $docs;
		...
		public function controlPagosById($dbcon,$pago_id,$DEBUG_STATUS)
		{
			...
			$sql="select distinct a.vp_codigo_viaje,a.vp_fecha_pago,a.vp_tipo_pago,a.vp_pic_pago,a.u_name,a.vp_observacion,a.vp_pago_verificado,
					sum(a.v_costo_viaje) v_costo_viaje from 
					(select vp.vp_viaje_id,vp.vp_codigo_viaje,vp.vp_fecha_pago,vp.vp_tipo_pago,vp.vp_pic_pago,u.u_name,vp.vp_observacion,
					vp.vp_pago_verificado,v.v_costo_viaje from z_viajespasajero vp,z_users u,z_viajes v where vp.vp_codigo_viaje='".$pago_id."' and  
					vp.vp_pass_id=u.u_id and vp.vp_viaje_id=v.v_id order by vp.vp_fecha_pago) a 
					group by a.vp_codigo_viaje,a.vp_fecha_pago,a.vp_tipo_pago,a.vp_pic_pago,a.u_name,a.vp_observacion,a.vp_pago_verificado";
			$docs=array();
			$count=0;
		...
		{
			$sql="update z_viajespasajero set vp_pago_verificado=".$estado.",vp_observacion='".$observacion."' where vp_codigo_viaje=".$Id;
			if(mysqli_query($dbcon,$sql) && mysqli_affected_rows($dbcon)>0)
            {
				return 0;
			}
		...
		public function terminateViaje($dbcon,$codigo_viaje,$userId,$DEBUG_STATUS)
		{
			//session_start();
			mysqli_autocommit($dbcon,FALSE);
			$err_code=1;
			$sql = "UPDATE z_viajespasajero SET vp_estado_viaje=4 WHERE vp_viaje_id=$codigo_viaje and vp_pass_id=$userId";
			if($DEBUG_STATUS)
				echo '$sql-1::'.$sql.'<br>';
	        if(mysqli_query($dbcon,$sql) && mysqli_affected_rows($dbcon)>0)
	        {
		...
		public function pagarConductor($dbcon,$id,$codigo_viaje,$docPago,$fechaPago,$DEBUG_STATUS)
		{
			...
			mysqli_autocommit($dbcon,FALSE);
			$err_code=1;
			$sql = "UPDATE z_viajespasajero SET vp_estado_viaje=7,vp_doc_paymet_to_client='".$docPago."',vp_dt_payment_to_client=date_format('".$fechaPago."','%Y-%m-%d') 
			WHERE vp_id=".$id;
			if($DEBUG_STATUS)
				echo '$sql-1::'.$sql.'<br>';
	        if(mysqli_query($dbcon,$sql) && mysqli_affected_rows($dbcon)>0)
	        {
	        	...
	        	$sql = "update z_viajes set v_estado=7 where v_id=$codigo_viaje and 
