<?php
	...
	class controller
	{
		...
		public function getClientTipo($dbcon,$id_cliente,$DEBUG_STATUS)
		{
			$sql="select tipo_cliente from p_clientes where id=".$id_cliente;
			$tipo_cliente=-1;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
		...
		{
			$sql="select u.id userid, u.perfil_id,u.nombre user_name,u.email,c.nombre client_name,c.id clientid,c.habilitado,c.tipo_cliente,gestion_cliente  
            ...
            {            	
            	if($row = mysqli_fetch_assoc($result)) 
            	{
					...
					$tipo_cliente=$row["tipo_cliente"];
					$gestion_cliente=$row["gestion_cliente"];
            	}
		        ...
		        if(mysqli_query($dbcon,$sql))
		        {
		            ...
		            else
		            {
						...
						$_SESSION["gestion_cliente"]=$gestion_cliente;
		...
		public function getMenu($dbcon,$DEBUG_STATUS)
		{
			if(isset($_SESSION["client_id"]) && $_SESSION["client_id"]==1)
				...
				$sql="select sec,param,nombre,url,id,habilitado from p_menu and id!=22 order by sec";
			$menu=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
		...
		//11-0,
		public function addSolicitud($dbcon,$equipo_desc,$client_id,$ciudad_id,$sucursal_id,$sala_id,$service_id,$id_equipos,$obser,$id_tecnico,$DEBUG_STATUS)
		{
			$id=mt_rand();
			mysqli_autocommit($dbcon,FALSE);
	        ...
	        if(mysqli_query($dbcon,$sql))
	        {
	        	if($service_id>1 && $service_id<7 && isset($id_equipos))
	        	{
		...
		public function getUsersBySala($dbcon,$ciudad_id,$sucursal_id,$sala_id,$DEBUG_STATUS)
		{
			...
			if($_SESSION["user_perfil"]==42)////Fase3-change perfil id
		...
		{
			$sql="select id, nombre from p_usuario where perfil_id=42 and habilitado=1 order by nombre";
			$tecnicos=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
		...
		public function getTecnicosPlan($dbcon,$DEBUG_STATUS)
		{
			$sql="select us.id,us.nombre user_name from p_sucursal su,p_clientes cl,p_ciudad ci,p_usuario us
					where su.id_tecnico=us.id and su.ciudad_id=ci.ID and ci.id_cliente=cl.id and us.perfil_id=42";
			if($_SESSION["client_id"]>1)
		...
		{
			$sql="select pp.id, pp.nombre_archivo,(select nombre from p_usuario pu where pu.id=pp.id_tecnico) tecnico,
					(select nombre from p_clientes pc where pc.id=id_cliente) cliente from p_planificaciones pp where pp.id>0";
			if($_SESSION["client_id"]==1 && $_SESSION["user_perfil"]<=2)
			{
				if($client_id!=99)
					$sql=$sql." and pp.id_cliente=".$client_id;
				if($tecnico_id!=99)
					$sql=$sql." and pp.id_tecnico=".$tecnico_id;
			}
			else if($_SESSION["client_id"]==1 && $_SESSION["user_perfil"]>2)
			{
				if($client_id!=99)
					$sql=$sql." and pp.id_cliente=".$client_id;
				$sql=$sql." and pp.id_tecnico=".$_SESSION["user_id"];
			}
			else if($_SESSION["client_id"]>1)
			{
				$sql=$sql." and pp.id_cliente=".$_SESSION["client_id"];
				if($tecnico_id!=99)
					$sql=$sql." and pp.id_tecnico=".$tecnico_id;
			}
            ...
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$plan[$count] = array($row["id"],$row["nombre_archivo"],$row["tecnico"],$row["cliente"]);
					$count++;
				}
			}
			return $plan;
		...
		{
			$sql="select distinct cl.id,cl.nombre client_name  from p_sucursal su, p_ciudad ci,p_clientes cl,p_usuario pu
					where su.id_tecnico=pu.id and su.ciudad_id=ci.ID and ci.id_cliente=cl.id";
			if($_SESSION["client_id"]>1)
				$sql=$sql." and cl.id=".$_SESSION["client_id"]." order by cl.nombre";
			else if($_SESSION["client_id"]==1 && $_SESSION["user_perfil"]<=2)
				$sql=$sql." order by cl.nombre";
			else if($_SESSION["client_id"]==1 && $_SESSION["user_perfil"]==42)
				$sql=$sql." and pu.id=".$_SESSION["user_id"]." order by cl.nombre";
			$clients=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$clients[$count] = array($row["id"],$row["client_name"]);
					$count++;
				}
			}
			return $clients;
		...
		{
			$sql="select distinct su.id_tecnico,pu.nombre tecnico  from p_sucursal su, p_ciudad ci,p_clientes cl,p_usuario pu
					where su.id_tecnico=pu.id and su.ciudad_id=ci.ID and ci.id_cliente=cl.id and pu.perfil_id=42";
			if($client_id!=99 && $_SESSION["client_id"]>1)
				$sql=$sql." and cl.id=".$_SESSION["client_id"]." order by cl.nombre";
			else if($client_id!=99 && $_SESSION["client_id"]==1 && $_SESSION["user_perfil"]<=2)
				$sql=$sql." order by cl.nombre";
			else if($client_id!=99 && $_SESSION["client_id"]==1 && $_SESSION["user_perfil"]==42)
				$sql=$sql." and pu.id=".$_SESSION["user_id"]." order by cl.nombre";
			$tecnicos=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$tecnicos[$count] = array($row["id_tecnico"],$row["tecnico"]);
					$count++;
				}
			}
			return $tecnicos;
