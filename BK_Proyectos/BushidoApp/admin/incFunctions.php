	...
	*/
	...
	if(!function_exists('getTableList')){
		function getTableList($skip_authentication = false){
			$arrTables = array(   
				'reglamento' => 'Lineamientos del Bushido',
				'puntos' => 'Mi Puntaje',
				'impactos' => 'Impactos',
				'persona' => 'Persona'
			);
	...
	function getThumbnailSpecs($tableName, $fieldName, $view){
		if($tableName=='impactos' && $fieldName=='imagen' && $view=='dv')
	...
	function logOutUser(){
		...
		if(isset($_COOKIE['bushidoapp_rememberMe'])){
			setcookie('bushidoapp_rememberMe', '', time()-42000);
		}
		session_destroy();
	}
	########################################################################
