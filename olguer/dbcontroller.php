<?php
	...
	class controller
	{
		...
		{
			$sql="select tipo_cliente from p_clientes where id=".$id_cliente.' and habilitado=1';
			$tipo_cliente=-1;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
		...
		public function getClientes($dbcon,$DEBUG_STATUS)
		{
			...
			if(isset($_SESSION["client_id"]) && $_SESSION["client_id"]==1)
			{
				$sql="select cl.id id_cliente,cl.nombre nombre_cliente,cl.id_ciudad ,cl.telefono,cl.celular,
					...
					from p_clientes cl,p_ciudad cd,p_usuario p  
					where p.client_id=cl.id and p.perfil_id in (1,2) and 
					cl.id_ciudad=cd.id and cl.habilitado=1 order by cl.nombre";
			}
			else
			{
				$sql="select cl.id id_cliente,cl.nombre nombre_cliente,cl.id_ciudad ,cl.telefono,cl.celular,
					...
					from p_clientes cl,p_ciudad cd,p_usuario p 
					where p.client_id=cl.id and p.perfil_id in (1,2) and 
					cl.id_ciudad=cd.id and cl.id=".$_SESSION["client_id"]." and cl.habilitado=1 order by cl.nombre";
			}
			$cliente=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
		...
		public function getClientById($dbcon,$client_id,$DEBUG_STATUS)
		{
			$sql="select cl.id id_cliente,cl.nombre nombre_cliente,cl.id_ciudad,cl.telefono,cl.celular,
					...
					cl.id_ciudad=cd.id and cl.id=".$client_id." and cl.habilitado=1 order by cl.nombre";
			$cliente=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
		...
		public function aprobarCliente($dbcon,$id_cliente,$is_approved,$obser,$tipo_cliente,$DEBUG_STATUS)
		{
			$sql="select cl.id id_cliente,cl.nombre nombre_cliente,cl.id_ciudad,cl.telefono,cl.celular,
					...
					from p_clientes cl,p_ciudad cd,p_usuario p 
					where p.client_id=cl.id and 
					cl.id_ciudad=cd.id and cl.id=".$id_cliente." and cl.habilitado=1 order by cl.nombre";
			$cliente=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				if($row = mysqli_fetch_assoc($result)) 
				{
					...
					if(mysqli_query($dbcon,$sql))
			        {
						...
						$subject = 'SISTEC NIPRO - VERIFICACION DE CUENTA';
		...
		{
			$sql="select id, nombre from p_clientes where nombre = '".strtoupper($nombre_cliente)."' and habilitado=1";
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) == 0)
            {
				        ...
				        if(mysqli_query($dbcon,$sql))
				        {
					        ...
					        if(mysqli_query($dbcon,$sql))
					        {
						        ...
						        if(mysqli_query($dbcon,$sql))
						        {
									...
									$subject = 'SISTEC NIPRO- REGISTRO DE CUENTA';
		...
		public function loginUser($dbcon,$user_email,$user_password,$DEBUG_STATUS)
		{
			$sql="select u.id userid, u.perfil_id,u.nombre user_name,u.email,c.nombre client_name,c.id clientid,c.habilitado,c.tipo_cliente,gestion_cliente  
				...
				where u.email = '".$user_email."' and u.password='".$user_password."' and u.habilitado=1 and u.client_id=c.id and c.habilitado=1";
			////echo $sql.'<br>';
			$updStatus=0;
			mysqli_autocommit($dbcon,FALSE);
			//$usr=array();
