<?php
	...
	if(isset($_GET["dojob"]))
	{
		...
		*/
		...
		else if($_GET["dojob"]==9 && $_GET["metodo"]==6)
		{
			//$users = $controller->getTecnicosParaServicio($databasecon,$_GET["ciudad_id"],$DEBUG_STATUS);
			$users = $controller->getTecnicosByClientType($databasecon,$_GET["client_type"],$DEBUG_STATUS);
		...
		else if($_GET["dojob"]==10 && $_GET["metodo"]==11)
		{
			//echo '<br><br><br><br>NOMBRE:'.$_GET["equipo_nombre"];
			$informeDtl = $controller->buscarInformesVisitasControl($databasecon,$_GET["client_id"],$_GET["ciudad_id"],$_GET["sucursal_id"],$_GET["sala_id"],$DEBUG_STATUS);	
			if(isset($informeDtl) && count($informeDtl)>0)
				echo '<label for="permisos">SE ENCUENTRA '.count($informeDtl).' SUCURSALES</label><br>';
			else
				echo '<label for="permisos">SE ENCUENTRA 0 SUCURSALES</label><br>';
			if($informeDtl>0)	
			{
				?>
				...
				<div class="row tbl_row_heading">
						...
						if(isset($informeDtl) && count($informeDtl)>0)
						{
							for($x=0;$x<count($informeDtl);$x++)
							{
						?>
								<div class="row tbl_row_data_static" style="font-size:12px">
									<div class="col-sm-1">
										<?php echo $informeDtl[$x][0];?>
									</div>				
									<div class="col-sm-3">									
										<?php echo $informeDtl[$x][1];?>
									</div>
									<div class="col-sm-3">
										<?php echo $informeDtl[$x][2];?>
									</div>
									<div class="col-sm-3">									
										<?php echo $informeDtl[$x][3];?>
									</div>
									<div class="col-sm-2 text-center">
										<a href=informeVisitas.php?eid=<?php echo $informeDtl[$x][0];?> target="_blank"><span class="glyphicon glyphicon-folder-open"></span></a>
									</div>
								</div>
						<?php	
								}	
		...
		{
			//echo '20-<br>';
			$err = $controller->addSolicitud($databasecon,$_GET["equipo_desc"],$_GET["client_id"],$_GET["ciudad_id"],$_GET["sucursal_id"],$_GET["sala_id"],$_GET["service_id"],$_GET["id_equipos"],$_GET["obser"],$DEBUG_STATUS);	
			//echo $err.'<br>';
			if($err>0)	
			{
				//echo 'SOLICITUD REGISTRADO CORRECTAMENTE. NRO-SOLICITUD:<h3>'.$err.'</h3>';
				echo 'SOLICITUD REGISTRADO CORRECTAMENTE. NRO-SOLICITUD:'.$err. "\r\n";
			}
			else
		...
		{
			$data = $controller->getPlanificaciones($databasecon,$_GET["client_id"],$_GET["tecnico_id"],$DEBUG_STATUS);	
			if(isset($data) && count($data)>0)
				echo '<label for="permisos">SE ENCUENTRA '.count($data).' PLANIFICACIONES</label>';
			else
				echo '<label for="permisos">SE ENCUENTRA 0 PLANIFICACIONES</label>';
			?>
				<div class="row">
					<div class="col-sm-12">
			            <br><br>
						...
						?>
							<div class="row tbl_row_data" style="font-size:12px">
								...
								<div class="col-sm-1">
									<a href=<?php echo $data[$x][0];?> target="_blank"><span class="glyphicon glyphicon-list-alt"></span></a>
								</div>
							</div>
						<?php
							}
		...
		else if($_GET["dojob"]==14 && $_GET["metodo"]==1)
		{
			//$estado=$_GET["estadoPeticion"];
		    $client=$_GET["clientId"];
		    //$dt_ini=$_GET["dt_ini"];
		    //$dt_fin=$_GET["dt_fin"];
			$clientes = $controller->getReport4ClientsData($databasecon,$client,$DEBUG_STATUS);	
			echo count($clientes);
			//echo "<option value=99>[99][TODOS]</option>";
		...
		else if($_GET["dojob"]==15 && $_GET["metodo"]==0)
		{
			$serviceId=$_GET["serviceId"];
		    $clientes = $controller->getClientesEnBaseDelServicio($databasecon,$serviceId,$DEBUG_STATUS);	
			//echo count($clientes);
			echo "<option value=9999>[9999][TODOS]</option>";
			for($x=0;$x<count($clientes);$x++)
				echo "<option value=".$clientes[$x][0].">[".$clientes[$x][0]."] ".$clientes[$x][1]."</option>";
