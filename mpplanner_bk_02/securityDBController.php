<?php
	class securityDBController
	{
		...
		{
			if($id==0)
				$sql="SELECT id, name, enabled FROM profile order by name";
			else
				$sql="SELECT id, name, enabled FROM profile where id=".$id." order by name";
			//echo $sql;
			$txn=array();
			$count=0;
			$result = mysqli_query($dbcon,$sql);
            if(mysqli_num_rows($result) > 0)  
            {
				while($row = mysqli_fetch_assoc($result)) 
				{
					$txn[$count] = array($row["id"],$row["name"],$row["enabled"]);
					$count++;
				}
			}
			return $txn;
