...
elseif ($action == 'findcontact')
{
    ...
    if (mysql_num_rows($result) > 0)
    {
        ...
        if (mysql_num_rows($result) > 0)
        {
            ...
            $html .=  "</table>\n";
            $html .= "<p align='center'><a href='contact_add.php?name=".urlencode(htmlspecialchars($search_string, ENT_QUOTES, $i18ncharset))."&amp;return=addincident'>{$strAddContact}</a></p>";
        ...
        else
        {
            ...
            echo "<p align='center'><a href=\"contact_add.php?name=".urlencode(htmlspecialchars($search_string, ENT_QUOTES, $i18ncharset))."&amp;return=addincident\">{$strAddContact}</a></p>";
        }
...
elseif ($action=='incidentform')
{
    ...
    if (empty($updateid))
    {
        ...
        // Don't show inventory section if none available for contact
...
elseif ($action == 'assign')
{
    ...
    if ($type == "support" || $type == "free")
    {
        ...
        $timeoffset = clean_int($_REQUEST['timeoffset']);
        ...
        if ($errors == 0)
        {
            ...
            if (!empty($updateid))
            {
                ...
                // remove from tempincoming to prevent build up of cruft
            ...
            $sql .= "VALUES ('{$incidentid}', '{$sit[2]}', 'slamet', '{$now}', '{$sit[2]}', '1', 'show', 'opened','" . clean_lang_dbstring($_SESSION['syslang']['strIncidentIsOpen'])."')";
            mysql_query($sql);
            if (mysql_error()) trigger_error("MySQL Query Error ".mysql_error(), E_USER_ERROR);
            ...
            plugin_do('incident_created_suggest_user');
...
elseif ($action == 'reassign')
{
    ...
    mysql_query($sql);
    if (mysql_error()) trigger_error("MySQL Query Error ".mysql_error(), E_USER_ERROR);
    plugin_do('incident_add_reassign');
