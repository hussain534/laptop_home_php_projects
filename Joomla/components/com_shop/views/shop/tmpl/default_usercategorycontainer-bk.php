<?php
    ...
    $sql = "select ri.rn_id from rn_items ri, rn_clients rc WHERE ";
    if($category!="All")
        $sql=$sql."rc.rn_category='".$category."' and ";
    $sql=$sql."ri.rn_client_id=rc.rn_client_id";
    ...
    $sql = "select ri.rn_id,ri.rn_client_id,ri.rn_item_name,rc.rn_client_name,ri.rn_price,ri.rn_rate item_rate,
            rc.rn_show_price,rc.rn_category,rn_offer,rn_offer_start_dt, rn_offer_end_dt from rn_items ri, rn_clients rc where ";
     if($category!="All")
        $sql=$sql."rc.rn_category='".$category."' and ";
    $sql=$sql."ri.rn_client_id=rc.rn_client_id";   
    ...
    {
        if($orderBy==1)
            $sql=$sql." order by ri.rn_price asc";
        else if($orderBy==2)
            $sql=$sql." order by ri.rn_price desc";
        else if($orderBy==3)
            $sql=$sql." order by ri.rn_rate asc";
        else if($orderBy==4)
            $sql=$sql." order by ri.rn_rate desc";
    }
    else
    {
        $orderBy=1;
		...
		<div class="row">
		    ...
		    <div class="col-sm-12">
		      	<div class="navi" style="width:90%;">
                    <?php 
                    if($current_page>0) { ?>
                    <a href=index.php?view=shop&layout=userCategoryContainer&pagecount=0&category=<?php echo $category; ?>&orderBy=<?php echo $orderBy;?> style="float:left;padding:0 15px;">First</a>
                    <?php }
                    else { ?>
                    <a href="#" style="float:left;padding:0 15px;color:#ccc;cursor:default;">First</a>
                    <?php }
                    if($current_page>0) { ?>
                    <a href=index.php?view=shop&layout=userCategoryContainer&pagecount=<?php echo $current_page-1; ?>&category=<?php echo $category; ?>&orderBy=<?php echo $orderBy;?> style="float:left;padding:0 15px;">Prev</a>
                    <?php } 
                    else { ?>
                    <a href="#" style="float:left;padding:0 15px;color:#ccc;cursor:default;">Prev</a>
                    <?php }
                    if($current_page<$last_page) { ?>
                    <a href=index.php?view=shop&layout=userCategoryContainer&pagecount=<?php echo $current_page+1; ?>&category=<?php echo $category; ?>&orderBy=<?php echo $orderBy;?> style="float:left;padding:0 15px;">Next</a>
                    <?php } 
                    else { ?>
                    <a href="#" style="float:left;padding:0 15px;color:#ccc;cursor:default;">Next</a>
                    <?php }
                    if($current_page<$last_page) { ?>
                    <a href=index.php?view=shop&layout=userCategoryContainer&pagecount=<?php echo $last_page; ?>&category=<?php echo $category; ?>&orderBy=<?php echo $orderBy;?> style="float:left;padding:0 15px;">Last</a>
                    <?php } 
                    else { ?>
                    <a href="#" style="float:left;padding:0 15px;color:#ccc;cursor:default;">Last</a>
                    <?php }
