<?php
    ...
    else
    {
        ...
        if(isset($_POST['submitted']))
    	{
    		...
    		$sqlDelete = "DELETE FROM rn_order_management WHERE order_num=".$_GET['orderId'];
    		...
    		for($x=0;$x<count($dataItems)-1;)
    		{
    			...
    			$sqlInsert = "INSERT INTO rn_order_management(order_num,user_id,user_name,item_id,item_name,client_id,client_name,
    							price_per_unit,total_units,total_price,registration_portal,status_portal)
    						  VALUES(".$_GET['orderId'].",'".$_SESSION['userid']."','".$_SESSION['userName']."',".$data[0].",'".$data[1]."',".$data[2].",'".$data[3]."',
    						  		".$data[4].",".$data[5].",".$data[6].",now(),1)";
				//echo $sqlInsert;
    			if(mysqli_query($dbcon,$sqlInsert))
    			{
    				$sqlConsultDir = "select rn_calle_principal,rn_numeration_casa,rn_intersection,rn_referencia,rn_cuidad
								from rn_order_address where rn_order_id=".$_GET['orderId'];
					...
					{
						$sqlInsertDir = "INSERT INTO rn_order_address VALUES(".$_GET['orderId'].",
			...
			{
				$sqlUpdate = "UPDATE rn_order_management SET STATUS_PORTAL=1 WHERE order_num=".$_GET['orderId'];
				if(mysqli_query($dbcon,$sqlUpdate))
    			{
					...
					$subject = "MODIFY ORDER REQUEST - ".$_GET['orderId'];
					$txt = "Hello ".$_SESSION['userName']."!\n\n";
					$txt=$txt."You modify your order :".$_SESSION['Order_id'].".\n\n";
					$txt=$txt."NOTE: If you didnot placed or modify this order, please contact to our customer care.\n";
					$headers = "From: webmaster@shop534.com";
					$res=mail($to,$subject,$txt,$headers);
    				$messsage="Your order is confirmed.Your Order ID is:".$_GET['orderId'].". Please keep this order 
...
?>
		<form method="post" action=index.php?view=shop&amp;layout=orderedit&orderId=<?php echo $_GET["orderId"];?>>
			...
			<div class="container">
					...
					<table class="table" id="t0">
						...
						<tbody>
							...
							<?php
									...
									total_price FROM rn_order_management WHERE order_num=".$_GET['orderId'];
							...
							<tr>
								...
								<td><a href=index.php?view=shop&layout=productcancel&prodId=<?php echo $rowSelect['item_id'];?>&orderId=<?php echo $_GET['orderId'];?> class="submit_button" onclick="return confirmDeleteProd('<?php echo $rowSelect['item_name'];?>');"><button type="button" class="btn btn-success" title="Click to delete this product from cart"><span class='glyphicon glyphicon-remove'></span></button></a></td>
							</tr>
							<?php
										}
									}
				...
				<div class="row order">
					<div class="col-sm-12 order_title" id="manageCartBtns">
						...
						<a href=index.php?view=shop&layout=cartcancel&orderId=<?php echo $_GET['orderId'];?> id="cancelCart" class="submit_button" onclick="return cancelCart1();"><button type="button" class="btn btn-success" title="Click to cancel your order">CANCEL YOUR ORDER</button></a>						
					</div>
				</div>
			</div>
		</form>
