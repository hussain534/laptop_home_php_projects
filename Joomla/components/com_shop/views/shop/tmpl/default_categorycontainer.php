<?php
    ...
    $sqlVisit = "insert into rn_visits(ip,user_id,page_id,visited_on)
                values ('".$_SERVER['REMOTE_ADDR']."','".$user_visit."','".$pageTitle.':'.$category."',now())";
    ...
    $sql = "select ri.rn_id from rn_items ri, rn_clients rc WHERE  ";
    $sql=$sql."ri.rn_item_category='".$category."' and ri.rn_enable='Y' and ";
    $sql=$sql."ri.rn_client_id=rc.rn_client_id";
    ...
    $sql = "select ri.rn_id,ri.rn_client_id,ri.rn_item_name,rc.rn_client_name,ri.rn_price,rn_stock_qty,rn_delivery_time,
            ...
            end as rn_offer, rn_offer_start_dt, rn_offer_end_dt from rn_items ri, rn_clients rc where ";
    $sql=$sql."ri.rn_item_category='".$category."' and ri.rn_enable='Y' and ";
    $sql=$sql."ri.rn_client_id=rc.rn_client_id order by item_rate desc ";  
  ...
  <div class="row">
    ...
    <div class="col-sm-12">
      <div class="navi" style="width:90%;">
          <?php 
          if($current_page>0) { ?>
          <a href=index.php?view=shop&layout=categorycontainer&pagecount=0&category=<?php echo $category; ?> style="float:left;padding:0 15px;">First</a>
        <?php }
        else { ?>
        <a href="#" style="float:left;padding:0 5px;color:#ccc;cursor:default;">First</a>
        <?php }
        if($current_page>0) { ?>
        <a href=index.php?view=shop&layout=categorycontainer&pagecount=<?php echo $current_page-1; ?>&category=<?php echo $category; ?> style="float:left;padding:0 15px;">Prev</a>
        <?php } 
        else { ?>
        <a href="#" style="float:left;padding:0 5px;color:#ccc;cursor:default;">Prev</a>
        <?php }
        if($current_page<$last_page) { ?>
        <a href=index.php?view=shop&layout=categorycontainer&pagecount=<?php echo $current_page+1; ?>&category=<?php echo $category; ?> style="float:left;padding:0 15px;">Next</a>
        <?php } 
        else { ?>
        <a href="#" style="float:left;padding:0 5px;color:#ccc;cursor:default;">Next</a>
        <?php }
        if($current_page<$last_page) { ?>
        <a href=index.php?view=shop&layout=categorycontainer&pagecount=<?php echo $last_page; ?>&category=<?php echo $category; ?> style="float:left;padding:0 15px;">Last</a>
        <?php } 
        else { ?>
                  ...
                  <div class="panel">
                    ...
                    <div class="panel-body" title="Click to view this product details">
                      ...
                      <?php 
                        ...
                        {
                      ?>
                      <img src=<?php echo JURI::root().'media/com_shop/'; ?>images/clients/<?php echo $row["rn_client_id"];?>/<?php echo $row["rn_client_id"].'-'.$row["rn_id"].'.jpg';?> style="width:100%;max-width: 300px;max-height: 300px">
                      <?php
                        }
                        else
                        {
