<?php
    ...
    {   
        ...
        if(strcmp($fecha_conciliacion, $fecha_archivo)==0)
        {
            ...
            if(strcmp($integrador_archivo, str_pad($_POST["integrador_carga"],4,"0",STR_PAD_LEFT))==0)
            {
                ...
                $sql="select id id_conciliacion from c_conciliacion where DATE_FORMAT(fecha_venta,'%Y%m%d')='".$fecha_conciliacion."' 
                        and id_integrador=".$_POST["integrador_carga"]." and habilitado in (1,3,4,5,6)";
                $id_conciliacion=0;
                $resultSet = mysqli_query($databasecon,$sql);
                if(mysqli_num_rows($resultSet) > 0)  
                {
                ...
                $sql="update c_ventas_interna set id_conciliacion=0,habilitado=1, fecha_conciliacion=null where id_conciliacion=".$id_conciliacion;
                if(mysqli_query($databasecon,$sql))
                {
                    $updStatus = 1;
                }
                ...
                if($updStatus==2)
                {
                    ...
                    error_log("Valor!".count($registros), 0); //0:error.log, 1:email,2:NA, 3:file
    ...
    else if($_GET["proceso"]==3 && $_GET["task"]==1)
    {
        ...
        if($_SESSION["user_empresa"]==0)
            echo 'conciliacion.php';
        else
            echo'subirArchivoConciliacion.php';
    ...
    else if($_GET["proceso"]==3 && $_GET["task"]==2)
    {
        ...
        $result=$result."<span class='glyphicon glyphicon-floppy-saved'></span>CANTITAD DE TRANSACCIONES INTEGRADOR:".count($txn_integradores)."<br>";
        $result=$result."<span class='glyphicon glyphicon-floppy-saved'></span>CANTITAD DE TRANSACCIONES ETAPA:".count($txn_ventas)."<br>";
        ...
        if(strlen($result)>0)
        {
            ...
            {               
                if($row = mysqli_fetch_assoc($resultSet)) 
                {
                    ...
                    $to = $email;
                    $subject = 'METROWIFI CONCIL - RESULTADO CONCILIACION : '.$_SESSION["FECHA_CONCILIACION"];
                    $txt = '¡HOLA, '.$nombre.'!'."<br><br>";
                    $txt=$txt.'Se ha realizado su conciliacion en METROWIFI CONCIL de los ventas del '.$_SESSION["FECHA_CONCILIACION"]."<br><br>";
        ...
        if(strlen($result)>0)
        {
            ...
            {               
                if($row = mysqli_fetch_assoc($resultSet)) 
                {
                    ...
                    $subject = 'METROWIFI CONCIL - RESULTADO CONCILIACION : '.$_SESSION["FECHA_CONCILIACION"];
                    $txt = '¡HOLA, '.$nombre.'!'."<br><br>";
                    $txt=$txt.'Se encuentra inconsistencias en concil, pero se ha realizado su conciliacion de manera forzada en METROWIFI CONCIL de los ventas del '.$_SESSION["FECHA_CONCILIACION"]."<br><br>";
                    $txt=$txt.'RESULTADOS::'."<br><br>";
                    $txt=$txt.$result."<br><br>";
                    $txt=$txt.'Si tienes alguna pregunta o necesitas ayuda, puedes ponerte en contacto con nosotros en support@conciliacion-metrowifi.etapa.net.ec'."<br><br>";
                    $txt=$txt.'MUCHAS GRACIAS'."<br><br>";
    ...
    else if($_GET["proceso"]==3 && $_GET["task"]==4)
    {
        ...
        //fecha_venta='".$_POST["consulta_fecha_conciliacion"]."'
        ...
        $sql="update c_ventas_interna cvi set cvi.habilitado=9 where fecha_venta='".$_GET["consulta_fecha_conciliacion"]."' and cvi.id_integrador=".$_GET["consulta_integrador"]." and cvi.habilitado in (1,3)";
        if(mysqli_query($databasecon,$sql))
        {
            /*$sql="insert into c_ventas_interna(id_txn_interna,id_txn_integrador,id_integrador,id_canal,id_plan,monto,fecha_venta, id_conciliacion, habilitado) 
                        select cvt.id_txn_interna,cvt.id_txn_integrador,cvt.id_integrador,cvt.id_canal,cvt.id_plan,cvt.monto,cvt.fecha_venta,0,cvt.habilitado from c_ventas_temp cvt 
                        where DATE_FORMAT(cvt.fecha_venta,'%Y%m%d')=DATE_FORMAT('".$_GET["consulta_fecha_conciliacion"]."','%Y%m%d') and cvt.id_integrador=".$_GET["consulta_integrador"]." and cvt.habilitado=1";*/
            $sql="insert into c_ventas_interna(id_txn_interna,id_txn_integrador,id_integrador,id_canal,id_plan,monto,fecha_venta, id_conciliacion, habilitado) 
                        select ve.InvoiceId,ve.TransactionId, ve.Id_integrador,1,ve.OfferCode,ve.OfferPrice,STR_TO_DATE(ve.CreateDate,'%Y-%m-%d'), 0, ve.habilitado from c_ventas_etapa ve where 
                        STR_TO_DATE(ve.CreateDate,'%Y-%m-%d')=DATE_FORMAT('".$_GET["consulta_fecha_conciliacion"]."','%Y-%m-%d') and ve.Id_integrador=".$_GET["consulta_integrador"]." and ve.habilitado=1 and ve.status='Active'";
            
            error_log("SQ-->".$sql, 0); //0:error.log, 1:email,2:NA, 3:file
            if(mysqli_query($databasecon,$sql))
            {
                mysqli_commit($databasecon);
    ...
    //cargar archivo ventas de alepo manualmente
    else if($_GET["proceso"]==3 && $_GET["task"]==5)
    {   
        $lineCtr=0;
        $fecha_conciliacion=str_replace("-","",$_POST["consulta_fecha_conciliacion"]);
        error_log("fecha_conciliacion!".$fecha_conciliacion, 0); //0:error.log, 1:email,2:NA, 3:file
        $_SESSION["FECHA_CONCILIACION"]=$fecha_conciliacion;
        //$_SESSION["INTEGRADOR"]=$_POST["integrador_carga"];
        error_log("fileToUpload!".$_FILES["fileToUpload"]["name"], 0); //0:error.log, 1:email,2:NA, 3:file
        $fecha_archivo=substr(str_replace("_","",$_FILES["fileToUpload"]["name"]),0,8);
        error_log("fecha_archivo!".$fecha_archivo, 0); //0:error.log, 1:email,2:NA, 3:file
        //$integrador_archivo=substr($_FILES["fileToUpload"]["name"],3,4);
        mysqli_autocommit($databasecon,FALSE);
        ...
        if(strcmp($fecha_conciliacion, $fecha_archivo)==0)
        {
                ...
                //BUSCAR SI EXISTE REGISTRO DE VENTAS PARA FECHA SELECCIONADO
                
                $sql="update c_ventas_etapa set habilitado=0 where CreateDate='".$_POST["consulta_fecha_conciliacion"]."'";
                if(mysqli_query($databasecon,$sql))
                {
                    $updStatus = 1;
                }
                $sql="update c_ventas_interna set habilitado=0 where fecha_venta='".$_POST["consulta_fecha_conciliacion"]."'";
                if(mysqli_query($databasecon,$sql))
                {
                    $updStatus = 1;
                }
        ...
        {
            $_SESSION["result"]= $_SESSION["result"]."<br><span class='glyphicon glyphicon-floppy-remove'></span>FECHA VENTAS SON DIFERENTES";
        }
    ...
    else if($_GET["proceso"]==3 && $_GET["task"]==6)
    {
        $_SESSION["FECHA_CONCILIACION"]=str_replace("-","",$_POST["consulta_fecha_conciliacion"]);;
        $_SESSION["INTEGRADOR"]=$_POST["consulta_integrador"];
        $url='subirArchivoConciliacion.php';
