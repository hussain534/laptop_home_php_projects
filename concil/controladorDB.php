<?php
	...
	class controladorDB
	{
		...
		public function loginUser($dbcon,$user_email,$user_password,$DEBUG_STATUS)
		{
            ...
            {            	
            	...
            	if(strcmp($usuario_red, "000")!=0)
            	{
					...
					$search ="samaccountname=".$usuario_red;
					error_log("usuario_red!".$usuario_red, 0);
					...
					$sr=ldap_bind($ds, $user , $psw); 
					if($sr)
						error_log("Authentication-TRUE!".$usuario_red, 0);
					else
						error_log("Authentication-FALSE!".$usuario_red, 0);
					// close connection
					ldap_close($ds);
					/*LDAP*/
            	}
		...
		public function registerUser($dbcon,$userNombre,$userEmail,$userPwd,$userTelefono,$userCelular,$userUbicacion,$userPerfil,$id_integrador,$usuarioRed,$DEBUG_STATUS)
		{
            ...
            if(mysqli_num_rows($result) == 0)
            {
            	$sql = "INSERT INTO c_login(nombre,email,telefono, celular, ubicacion, perfil,clave,creado_por, fecha_creacion,modificado_por,fecha_modificacion,id_client,habilitado,usuario_red) 
